...
# Призываем гримуар 'numpy' (np) - наш магический калькулятор для работы с числовыми массивами.
import numpy as np
...
import torch
# Призываем 'torch.nn' (nn) - главу из гримуара 'torch', хранящую чертежи всех строительных блоков для големов.
import torch.nn as nn
# Призываем 'MinMaxScaler' - духа-нормализатора, который "сжимает" числа в заданный диапазон.
from sklearn.preprocessing import MinMaxScaler
# Призываем 'matplotlib.pyplot' (plt) - нашего верного "магического художника" для визуализации данных.
...
# Мы начинаем чертеж нашего Голема-Оракула, наследуя его от базового чертежа всех големов 'nn.Module'.
class OracleLSTM(nn.Module):
    # В ритуале сотворения (`__init__`) мы описываем слои, из которых будет состоять наш голем.
    ...
    # В ритуале "прямого прохода" (`forward`) мы описываем, как данные проходят через слои голема.
    def forward(self, input_seq):
        ...
        # LSTM возвращает нам обработанную последовательность и свое новое состояние памяти.
        lstm_out, self.hidden_cell = self.lstm(input_seq.view(len(input_seq), 1, -1), current_hidden_cell)
        
        # Берем вывод только от *последнего* элемента последовательности и отправляем в линейный слой.
...
if __name__ == "__main__":
    ...
    # Приказываем Хранителю Летописей 'pandas' прочитать наш CSV-свиток и создать таблицу.
    ...
    # Призываем духа-нормализатора. 'feature_range' говорит "сжимай все числа в диапазон от -1 до 1".
    scaler = MinMaxScaler(feature_range=(-1, 1))
    # Приказываем духу "изучить и преобразовать" наши учебные данные. '.reshape(-1, 1)' - техническая руна для совместимости.
    ...
    # Превращаем наши numpy массивы в тензоры PyTorch, понятные Источнику Маны. '.float()' задает тип данных.
    ...
    # Призываем нашего Голема-Оракула по чертежу и отправляем на магический алтарь (`.to(DEVICE)`).
    model = OracleLSTM().to(DEVICE)
    # Выбираем "меру ошибки" (Среднеквадратичная ошибка), чтобы оценивать промахи голема.
    loss_function = nn.MSELoss()
    # Выбираем "наставника" (алгоритм Adam), который будет корректировать "разум" Оракула.
    ...
    for i in range(epochs):
        ...
        for seq, labels in zip(X_train, y_train):
            ...
            # Обнуляем память самого Голема перед тем, как показать ему новую "карточку".
        ...
        if (i+1) % 25 == 0:
            # Выводим номер эпохи и текущее значение ошибки. ':.8f' - форматирование до 8 знаков после запятой.
    ...
    print("\nНачинаем экзамен...")
    # Берем последние 'seq_length' дней из УЧЕБНЫХ данных как "затравку" для первого предсказания.
    ...
    # Запускаем цикл предсказаний на столько шагов, сколько у нас дней в "экзаменационной" части.
    for i in range(len(test_data)):
        ...
        with torch.no_grad():
            # Создаем пустой "сосуд" для Краткосрочной Памяти (h_n) на нужном устройстве.
            ...
            # Оракул делает предсказание следующего дня. '.item()' извлекает число из тензора.
            next_pred = model(seq).item()
            # Добавляем предсказание в конец нашей истории, чтобы использовать его для следующего шага.
            test_inputs.append([next_pred])

    # --- Акт 5: Оценка и Визуализация ---
    # Приказываем духу-нормализатору 'разжать' наши предсказанные данные обратно в понятный человеку масштаб.
    ...
    plt.plot(x[-len(actual_predictions):], actual_predictions, label='Предсказания Оракула', linestyle='--')
