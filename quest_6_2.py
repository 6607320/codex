...
full_dataset = load_dataset("PolyAI/minds14", name="en-US", split="train", trust_remote_code=True)
...
for sample in tqdm(full_dataset.select(range(50)), desc="Извлечение эссенций"):
    ...
    inputs = feature_extractor(audio_data, sampling_rate=16000, return_tensors="pt").to("cuda")
    with torch.no_grad():
