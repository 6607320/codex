# === main.py (Версия для A/B Тестирования) ===
...
# Мы произносим заклинание 'pipeline', указывая его цель ('sentiment-analysis') и истинное имя духа, которого мы призываем.
model_a = pipeline(
...
# Мы снова используем заклинание 'pipeline', но на этот раз называем имя другого, альтернативного духа.
model_b = pipeline(
...
# Мы приставляем к нашему порталу 'Инструментатора', который будет собирать стандартные метрики и выставлять их на обозрение.
...
# Мы создаем новую тропу к нашему порталу, которая будет принимать послания методом POST.
@app.post("/analyze")
# Мы определяем ритуал, который будет происходить, когда кто-то ступит на эту тропу.
...
def validate_model():
    ...
    return {
        ...
        "model_tested": "A",
        # Мы записываем вычисленную точность его предсказаний, его магическую силу.
        "accuracy": accuracy,
