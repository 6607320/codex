"""Квест 1.3: Первый Призыв — Дух Эмоций с помощью `pipeline`.

Этот пергамент — ритуал первого посвящения в магию `transformers`. Его
главная цель (МАКРО-контекст) — продемонстрировать самый высокоуровневый
и быстрый способ получения ценности от предобученной AI-модели.

Мы не строим Голема с нуля. Мы используем могущественный "амулет" `pipeline`,
который скрывает всю сложность ритуала (токенизацию, инференс, постобработку)
и немедленно выдает результат. Ритуал призывает 'Духа Эмоций', передает
ему на суд магическую фразу и получает его вердикт о ее тональности.

Этот артефакт является практическим воплощением урока, запечатанного в
`CODEX.md`, и доказывает, что для решения многих задач не нужно быть
Архимагом — достаточно знать, какой "амулет" использовать.
"""

# --- Акт 1: Подготовка Гримуара ---

# Призываем из гримуара "transformers"
# наш самый универсальный инструмент
#  - "pipeline".
