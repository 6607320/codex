...
Этот пергамент — кульминация Первого Свитка. Его главная цель
(МАКРО-контекст) — полностью демистифицировать "черный ящик" `pipeline`,
проведя весь ритуал предсказания (инференса) вручную.

Мы больше не полагаемся на "универсальный амулет". Вместо этого мы, как
истинные Техноманты, шаг за шагом воспроизводим его магию:
1. Призываем "Толмача" (`AutoTokenizer`) и самого "Голема" (`AutoModel`).
2. Превращаем человеческую речь в числовые "руны" (токенизация).
3. "Скармливаем" эти руны Голему, чтобы заглянуть в его "сырые мысли" (логиты).
4. Самостоятельно интерпретируем эти мысли, чтобы вынести финальный вердикт.

Этот квест доказывает, что инференс — это не волшебство, а понятный
инженерный конвейер, состоящий из трех этапов: пре-процессинг, инференс
и пост-процессинг.
"""
