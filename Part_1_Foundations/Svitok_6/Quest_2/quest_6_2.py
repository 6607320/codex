# === quest_6_2.py ===
# Имя этого пергамента, в котором мы создаем нашего собственного Голема.
...
# Священная цель нашего ритуала, объясняющая суть Переноса Знаний.
...
# `trust_remote_code=True` — это обязательное заклинание для запуска "свитка-инструкции" этого архива.
...
# Мы произносим заклинание `select(range(50))`, чтобы взять только первые 50 записей из архива.
...
# Мы "склеиваем" (`cat`) отдельные "эссенции" в один большой тензор для обучения.
...
# Мы передаем ему "настройки" нашего ученика (`classifier.parameters()`) для "подкрутки".
...
for epoch in tqdm(range(100), desc="Обучение классификатора"):
    ...
    # Шаг 3: Мы измеряем, насколько предсказание отличается от правильных ответов.
    loss = criterion(outputs, labels_tensor)
    # Шаг 4: Мы вычисляем, как именно нужно "подкрутить" настройки, чтобы исправиться.
    loss.backward()
    # Шаг 5: Мы используем "Волшебный Ключ", чтобы реально "подкрутить" настройки.
    optimizer.step()
