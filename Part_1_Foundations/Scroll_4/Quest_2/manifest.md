...
Именно в этой точке инженеры переходят на следующую ступень мастерства. Для решения этой проблемы используется более сложная магия, известная как **"Оптический Поток"** (**_`Optical Flow`_**). Эта технология не просто ищет разницу, а вычисляет векторы движения для каждого пикселя. Это позволяет алгоритму "понять", как движется общий фон (из-за смещения камеры), скомпенсировать это движение и уже после этого искать объекты, которые движутся _относительно_ этого фона. Это следующий, более высокий уровень понимания сцены в компьютерном зрении.
...
**_И это заставляет задуматься. Какие еще простые идеи, подобно сравнению пикселей, лежат в основе самых передовых технологий, меняющих наш мир сегодня?_**
