...
Этот пергамент делает наш первый шаг в настоящую видеоаналитику. Его главная
цель (МАКРО-контекст) — научить машину "видеть" изменения во времени с
помощью простейшей, но могущественной техники "вычитания кадров".

Ритуал берет два последовательных кадра из кинопленки и применяет к ним
магию попиксельного сравнения (`cv2.absdiff`). В результате создается
"карта движения" — образ, на котором подсвечены только те области, где
произошли изменения. Этот квест является фундаментом для понимания
принципов, лежащих в основе систем безопасности и контроля качества.
"""
...
frame1 = cv2.imread("../Quest_1/frames/frame_1.jpg")
frame2 = cv2.imread("../Quest_1/frames/frame_2.jpg")
