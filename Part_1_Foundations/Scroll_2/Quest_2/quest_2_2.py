...
Этот пергамент демонстрирует применение "Уха Всеслышания" к реальной задаче.
Его главная цель (МАКРО-контекст) — научиться работать с настоящими
аудио-данными и освоить магию потоковой загрузки из "Великой Библиотеки"
(`Hugging Face Hub`).

Мы не создаем аудио сами, а используем заклинание `load_dataset` с руной
`streaming=True`, чтобы открыть "портал" в архив и "выдернуть" оттуда
один-единственный аудио-свиток. Затем мы приказываем "Уху Всеслышания"
расшифровать его, доказывая, что наш дух готов к работе с реальными
артефактами.
"""

# --- Акт 1: Подготовка Гримуаров ---
# Мы начинаем первый магический акт: призыв необходимых знаний из великих
# библиотек.
...
# Из великой библиотеки `transformers`
#  мы призываем `pipeline` — наш универсальный "амулет",
...
# Оглашаем на кристалл (консоль) начало нашего ритуала, указывая на
# источник магии.
...
# 2. "clean" - Вторая руна: мы просим "чистую"
#  версию записей, без лишних помех.
# 3. split="test" - Третья руна: мы берем образец
#  из тестовой части архива (обычно самой компактной).
# 4. streaming=True - Ключевая руна экономии!
#  Мы не скачиваем весь архив на диск, а создаем "поток",
...
# Мы произносим заклинание `next(iter(...))`,
#  которое извлекает из бесконечного потока данных
# самый первый попавшийся образец и сохраняет его.
sample = next(iter(dataset))

# Призванный 'образец' — это магический контейнер (словарь).
#  Мы извлекаем из него сокровище
# под ключом "audio", где хранятся как сами звуковые волны, так и их
# характеристики.
audio_input = sample["audio"]

# Сообщаем об успешном завершении Акта 2: артефакт для исследования у нас
# в руках.
...
# Заклинание `.copy()` — это защитная руна, создающая копию данных.
