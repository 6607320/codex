...
Этот пергамент раскрывает магию аугментации — искусства сотворения
множества "иллюзорных копий" из одного-единственного артефакта. Его
главная цель (МАКРО-контекст) — научиться программно изменять изображения,
чтобы искусственно увеличить разнообразие "учебников" (датасетов).

Мы создаем "конвейер магических лизн" (`torchvision.transforms`), который
последовательно искажает, поворачивает и отражает исходный образ.
Этот квест доказывает, что аугментация — это не просто "фильтры", а
могущественный инструмент для борьбы с "переобучением", который учит
Голема распознавать суть объекта, а не запоминать конкретные пиксели.
"""
...
# `as T` — это краткое заклинание, создающее псевдоним `T`,
#  чтобы не писать длинное имя каждый раз.
import torchvision.transforms as T

# Мы призываем из гримуара `Pillow` (PIL) заклинание `Image`,
# которое позволяет нам работать с файлами изображений, вскрывать их и
# сохранять.
...
# Третий, самый важный акт: мы создаем последовательность магических
# трансформаций.
...
transform_pipeline = T.Compose(
    [
        ...
        # `p=1.0` — вероятность срабатывания 100%.
        #  Мы приказываем отражать этот образ всегда.
        T.RandomHorizontalFlip(p=1.0),
        # Третья "линза": `T.RandomRotation`.
        # Она поворачивает изображение на случайный угол в диапазоне
        #  от -45 до +45 градусов.
        T.RandomRotation(degrees=45),
        # Четвертая "линза": `T.ColorJitter`.
        # Она "искажает" цвета, случайным образом меняя яркость (`brightness`)
        #  и оттенок (`hue`).
...
# Четвертый, кульминационный акт: мы пропускаем наш артефакт через
# сотворенный конвейер.
