# === quest_3_2.py ===
# Имя этого пергамента, хранящего один из самых сложных ритуалов нашего Фундамента Ремесла.
...
# Важнейшее предупреждение: эта магия жадна до маны, и мы будем использовать тайные искусства для её укрощения.

# --- Акт 1: Подготовка Гримуаров ---
# Первый акт: мы призываем гримуары, содержащие необходимые нам знания.

# Призываем наш главный силовой гримуар `PyTorch`, источник всей магической энергии.
import torch
...
# Оглашаем начало ритуала. Предупреждаем Мага, что призыв потребует времени и места в хранилище.
print("Призываю Духа-Демиурга в режиме экономии...")

# Начинается само заклинание призыва Духа-Демиурга (всего конвейера творения).
# 1. "CompVis/stable-diffusion-v1-4" - Первая руна: мы называем точное имя Духа, которого хотим призвать.
...
# Великая руна `enable_model_cpu_offload()`: мы приказываем не держать всего гигантского Духа
# в драгоценной, быстрой памяти Кристалла Маны (GPU VRAM). Только активная часть Духа будет
# находиться на GPU, а остальные его части будут смиренно ждать в обычной, медленной памяти (CPU RAM).
# Это замедляет процесс творения, но делает его ВООБЩЕ ВОЗМОЖНЫМ на нашем Кристалле в 4ГБ.
pipe.enable_model_cpu_offload()

# --- Акт 4: Ритуал Творения ---
# Четвертый, кульминационный акт: мы направляем волю Духа и наблюдаем за творением.

# Мы создаем текстовое заклинание (промпт), которое описывает образ, что мы желаем сотворить.
...
# Сообщаем, что самый таинственный этап — сотворение образа из первозданного хаоса (шума) — начался.
...
# и Дух итеративно (по умолчанию 50 раз) "очищает" этот шум, руководствуясь нашим заклинанием,
# пока не проявится образ. `.images[0]` — результат это список образов, из которого мы берем первый.
