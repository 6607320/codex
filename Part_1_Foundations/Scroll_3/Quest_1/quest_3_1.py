...
Этот пергамент открывает "Свиток Искусства Иллюзий" (Computer Vision).
Его главная цель (МАКРО-контекст) — провести наш первый ритуал
классификации изображений, используя высокоуровневый "амулет" `pipeline`.

Мы призываем "Всевидящее Око" (`MobileNetV2`) — духа, обученного на
миллионах образов. Затем мы "скармливаем" ему случайную картинку из архива
и просим вынести вердикт о том, что на ней изображено. Этот квест также
является первым уроком о "сдвиге домена" — фундаментальной проблеме, когда
модель, обученная на одном типе данных, плохо работает на другом.
"""

# --- Акт 1: Подготовка Гримуаров ---
# Мы начинаем первый магический акт: призыв знаний и инструментов из
# великих библиотек.

# Из библиотеки `datasets` мы призываем
#  заклинание `load_dataset`, нашего "Библиотекаря",
# который умеет безопасно и эффективно призывать архивы с данными для
# обучения и тестов.
from datasets import load_dataset

# Из библиотеки `transformers` мы снова призываем `pipeline`
#  — наш универсальный "амулет",
# который скрывает сложность ритуалов и позволяет нам легко повелевать
# Големами и Духами.
from transformers import pipeline

# --- Акт 2: Призыв Магического Образа ---
# Начинается второй акт: мы материализуем объект для нашего исследования —
# изображение.
...
# CIFAR-10 — это древний и почитаемый архив образов,
...
# Призванный 'образец' (`sample`) — это контейнер.
#  Нас интересует его содержимое под ключом 'img',
...
# `dataset.info.features['label'].int2str(...)` — это заклинание,
# которое переводит это число в понятное слово (например, 'frog').
true_label = dataset.info.features["label"].int2str(sample["label"])
# Мы выводим на кристалл эту "истинную суть", чтобы позже сравнить её с
# вердиктом нашего Духа.
print(f"(Истинная суть образа, как говорят хроники: {true_label})")

# --- Акт 3: Ритуал Распознавания ---
# Третий, кульминационный акт: мы призываем Духа и просим его
# проанализировать образ.

# Оглашаем призыв нашего "Всевидящего Ока". При первом запуске скрипта Дух
# будет скачан из астрала.
...
# 2. model="google/mobilenet_v2_1.0_224" — имя призываемого Духа.
