...
Этот пергамент открывает "Свиток Управления Потоком Времени". Его главная
цель (МАКРО-контекст) — освоить Frame Sampling, фундаментальную технику
оптимизации для работы с видео.

Видео — это "жадный" до маны поток данных. Обрабатывать каждый кадр —
расточительно. Мы научимся управлять циклом обработки с помощью простого,
но могущественного заклинания остатка от деления (`%`), которое будет
служить "магическим фильтром", позволяя "выхватывать" из потока времени
лишь каждый N-ный кадр. Этот ритуал демонстрирует ключ к эффективной
работе с видео: не обрабатывать лишнего.
"""
