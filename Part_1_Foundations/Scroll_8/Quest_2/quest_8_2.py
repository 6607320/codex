...
Этот пергамент — кульминация Первой Части Кодекса. Его главная цель
(МАКРО-контекст) — продемонстрировать полный конвейер (pipeline) для
классификации видео с использованием магии Transfer Learning.

Мы не обучаем Голема "видеть" с нуля. Вместо этого мы проводим сложный
ритуал из пяти актов:
1. Создаем "учебник" из двух видео с разными действиями.
2. "Прореживаем" кадры из них для оптимизации.
3. Извлекаем из каждого кадра "визуальную эссенцию" с помощью "Всевидящего Ока".
4. Агрегируем (усредняем) "эссенции", чтобы получить "единый отпечаток" видео.
5. Обучаем нашего крошечного "Голема-Ученика" отличать один "отпечаток" от другого.

Этот квест доказывает, что, объединив знания из предыдущих Свитков, мы можем
решать сложные задачи, такие как распознавание действий.
"""
...
# Третий акт: мы создаем "учебник" для нашего Голема, извлекая суть из
# каждого видео.
...
for video_path in tqdm(video_files, desc="Извлечение видео-эссенций"):
    ...
    if not video_frames:
        # Мы произносим заклинание `continue`, чтобы перейти к следующему
        # видео.
        continue
    ...
    # Мы "скармливаем" всю пачку кадров "Настройщику", а затем "Всевидящему
    # Духу".
