...
# Второй акт: мы призываем могущественного духа, который будет извлекать для нас "эссенции".
...
for video_path in tqdm(video_files, desc="Извлечение видео-эссенций"):
    ...
    # Мы используем защитную руну: если видео было слишком коротким и мы не извлекли ни одного кадра, мы пропускаем его.
    if not video_frames:
