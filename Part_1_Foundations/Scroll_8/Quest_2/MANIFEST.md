...
Искусственный интеллект, который смотрит видео и понимает, что в нем происходит, кажется чем-то из области научной фантастики. На первый взгляд, это технология невероятной сложности, сродни _магии_. Как машина может отличить одно действие от другого — скажем, простое движение объекта вправо от его увеличения?
...
> _Мастер, мы усреднили «эссенции» всех кадров, чтобы получить «единый отпечаток» видео. Но не теряем ли мы при этом всю информацию о последовательности действий?_
...
Для этого используются архитектуры, изначально разработанные для обработки языка — например, рекуррентные нейронные сети (`RNN/LSTM`) или **Трансформеры**. Проводится удивительная параллель: эти продвинутые модели _«читают» последовательность «эссенций» кадров точно так же, как языковые модели читают последовательность слов в предложении_. Они способны улавливать их порядок, контекст и взаимосвязи, что позволяет им понимать сложные, многоэтапные действия.
...
_Теперь, когда вы знаете этот фундаментальный «рецепт», какую повседневную задачу вы бы доверили решать такому «Голему-Классификатору»?_
