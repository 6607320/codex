"""Подготовительный ритуал для Квеста 8.2.

Этот пергамент не является самостоятельным Квестом. Его единственная цель
(МАКРО-контекст) — **программно сотворить "учебник" (видео-датасет)** для
обучения классификатора действий в Квесте 8.2.

Мы используем магию OpenCV (`cv2`) для генерации двух коротких видео. Каждое
видео демонстрирует уникальное "действие": "движение вправо" и "увеличение".
Эти два артефакта, помещенные в разные папки (`action_A`, `action_B`),
создают чистый, контролируемый набор данных, идеальный для первого ритуала
Наставления Голема-Классификатора.
"""
...
# Второй акт: мы создаем "съемочную площадку" — структуру папок для наших
# фильмов.
...
# Мы измеряем его размеры, чтобы наша "сцена" (видео-кадр) была такого же
# размера.
...
for i in range(30):
    ...
    # Мы применяем эту матрицу к нашему "актеру" (`img`), создавая сдвинутый
    # кадр.
...
for i in range(30):
    ...
    # Мы записываем этот "приближенный" и обрезанный кадр на вторую
    # "кинопленку".
    out_B.write(cropped)
