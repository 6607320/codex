# === quest_5_3.py ===
# Имя этого пергамента, который будет занесен в Летопись как свидетельство экзамена.
...
# Священная цель нашего ритуала: провести экзамен для нашего ученика.
...
# Детальное описание шагов, которые приведут нас к цели.
...
# Мы призываем специальный чертеж `PeftModel` из гримуара `peft` для работы с "блокнотами".
...
# Второй акт: мы призываем "чистого" Голема и наделяем его знаниями из нашего "блокнота".
...
# Это КРИТИЧЕСКИ ВАЖНО: Голем и его "блокнот" должны существовать в одной "системе магии".
...
# Мы устанавливаем 'pad_token', как делали при обучении, для полной совместимости.
tokenizer.pad_token = tokenizer.eos_token

# Мы указываем точный путь к нашему артефакту — папке с обученным "магическим блокнотом".
...
# Мы переводим наш вопрос в числовые руны-тензоры и отправляем их на Кристалл Маны (GPU).
...
outputs = model.generate(
    ...
    # Руна №2: мы передаем "карту внимания", чтобы Голем знал, на какие руны смотреть.
    attention_mask=inputs["attention_mask"],
    # Руна №3: мы ограничиваем длину ответа 20-ю новыми токенами, чтобы он был кратким.
    max_new_tokens=20,
)
