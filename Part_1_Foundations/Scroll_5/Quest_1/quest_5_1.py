...
# Мы призываем заклинание `load_dataset`, нашего "Библиотекаря", из гримуара `datasets`.
from datasets import load_dataset

# --- Акт 2: Открытие "Портала" к Архиву ---
# Второй акт: мы устанавливаем связь с великим архивом знаний, не скачивая его целиком.

# Мы записываем в переменную точное имя архива в Великой Библиотеке (Hugging Face Hub).
archive_name = "databricks/databricks-dolly-15k"
# Мы оглашаем на кристалл (консоль) о начале ритуала и имени архива, к которому обращаемся.
...
# Третий акт: мы проходим через "портал" и извлекаем ровно столько знаний, сколько нам нужно.

# Мы оглашаем на кристалл, что начинаем процесс извлечения определенного числа записей.
print("\nИзвлекаю 100 страниц для нашего урока...")

# Мы создаем пустой "свиток" (список Python) для хранения наших учебных страниц.
small_dataset = []

# Мы запускаем ритуальный цикл `for` для итерации по нашему "порталу" `full_dataset`.
# `enumerate` — это "дух-счетовод", который для каждой записи (`sample`) также выдает ее порядковый номер (`i`).
for i, sample in enumerate(full_dataset):
    # Мы устанавливаем "магический барьер": если номер записи (`i`) достиг или превысил 100...
    if i >= 100:
...
# Финальный акт: мы изучаем структуру одной "страницы", чтобы понять, с чем имеем дело.

# Мы оглашаем на кристалл, что сейчас рассмотрим первую "страницу" нашего учебного свитка.
...
# Мы извлекаем и выводим на кристалл значение, хранящееся под ключом 'instruction'.
...
# Мы извлекаем и выводим на кристалл значение, хранящееся под ключом 'response'.
