...
Этот пергамент — один из самых могущественных ритуалов в Кодексе. Его
главная цель (МАКРО-контекст) — провести дообучение (fine-tuning) языковой
модели, используя передовую магию QLoRA, чтобы сделать ритуал возможным
даже на скромном Кристалле Маны.

Мы не переписываем весь "разум" Голема. Вместо этого, мы "сжимаем" его до
4-бит (`BitsAndBytesConfig`), а затем прикрепляем к его "мыслительным
центрам" крошечные "магические блокноты" (`LoRA`). Обучая только эти
"блокноты", мы эффективно адаптируем гигантскую модель под наши нужды,
доказывая, что Parameter-Efficient Fine-Tuning (PEFT) — это ключ к
демократизации магии LLM.
"""

# --- Акт 1: Подготовка Всех Гримуаров ---
# Первый акт: мы призываем все необходимые знания и инструменты для этого
# сложного ритуала.
...
# Из гримуара `peft` мы призываем заклинания для "Эффективной Адаптации"
# (LoRA).
...
from transformers import (
    ...
    AutoTokenizer,
    # Чертеж для создания "свитка-инструкции" по магическому сжатию
    # (квантизации).
    BitsAndBytesConfig,
    # Чертеж для "Помощника-Упаковщика", который грамотно формирует порции
    # данных для урока.
...
# Техническая руна: мы приказываем Переводчику использовать
#  символ "конца текста" (`eos_token`)
...
quantization_config = BitsAndBytesConfig(
    ...
    # Руна №2: Указываем использовать эффективный тип сжатия "Normal Float 4".
    bnb_4bit_quant_type="nf4",
    # Руна №3: Во время вычислений использовать 16-битную точность для баланса.
...
# Мы создаем "Магический Преобразователь" — заклинание (функцию),
#  которое будет "на лету"
...
def process_dataset(batch):
    ...
    texts = [
        ...
        f"Instruction: {instr}\n\nResponse: {resp}"
        # Проходимся одновременно по инструкциям и ответам в текущей порции данных
        # (`batch`).
...
# Третий акт: мы готовим Голема к обучению и "прикрепляем" к нему адаптеры
# LoRA.

# Мы произносим над сжатым Големом подготовительное заклинание, чтобы он
# был готов к адаптации.
...
# Четвертый, кульминационный акт: мы призываем Наставника и запускаем
# процесс обучения.
...
training_args = TrainingArguments(
    ...
    # Приказываем учить по одной "учебной карточке" за раз.
    per_device_train_batch_size=1,
    # Но накапливать "опыт" с 4-х карточек перед обновлением знаний.
    gradient_accumulation_steps=4,
    learning_rate=2e-4,  # Устанавливаем "скорость обучения".
    max_steps=250,  # Главная руна: урок длится ровно 250 шагов.
    # Приказываем Наставнику докладывать о прогрессе каждые 20 шагов.
    logging_steps=20,
)
...
trainer = Trainer(
    ...
    # По какому "умному порталу" с учебниками учить.
