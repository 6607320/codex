...
# Мы призываем чертеж `DiffusionPipeline` из гримуара `diffusers` — сложнейшего механизма для магии творения.
...
# Мы произносим заклинание `makedirs`, чтобы создать эту папку, если ее еще нет.
...
pipe = DiffusionPipeline.from_pretrained(
    ...
    # Мы приказываем загружать "мысли" (веса) модели в "легком" 16-битном формате.
    torch_dtype=torch.float16,
)
# Мы активируем ритуал "Призрачного Перемещения" для максимальной экономии VRAM.
pipe.enable_model_cpu_offload()

# --- Акт 4: Ритуал "Самосотворения" ---
# Четвертый, кульминационный акт: мы запускаем процесс творения наших учебных материалов.
