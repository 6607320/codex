...
# Мы призываем могущественный гримуар `cv2` (OpenCV), известный как "Набор Кинематографиста".
import cv2
...
# Третий, кульминационный акт: мы сравниваем два среза времени, чтобы найти разницу.
...
# Четвертый акт: мы обрабатываем полученную карту различий, чтобы сделать ее четкой и ясной.

# Шаг 4.1: Мы лишаем "Карту Различий" ее цветовой души, чтобы работать только с яркостью.
...
# Это упрощает дальнейший анализ, сводя три числа на пиксель (B, G, R) к одному.
...
# 2. `25` — порог. Все, что тусклее этого значения, будет считаться тьмой (незначительным шумом).
# 3. `255` — значение для света. Все, что ярче порога, станет чисто-белым (явным движением).
# 4. `cv2.THRESH_BINARY` — тип фильтра: "или тьма, или свет".
# `_` — это знак мудрого мага, который игнорирует ненужный побочный результат заклинания,
...
# Финальный акт: мы материализуем нашу "Карту Движения", чтобы ее можно было изучить.

# Заклинание `cv2.imwrite` сохраняет наш артефакт `motion_mask` на диск в виде файла.
