...
# Мы устанавливаем параметры обучения, адаптированные для нашего Кристалла Маны.
resolution = 256
# Устанавливаем размер порции (батча) для обучения.
train_batch_size = 1
# Устанавливаем, через сколько шагов накапливать и применять "опыт" (градиенты).
...
preprocess = transforms.Compose(
    ...
    [
        ...
        # Вторая "линза": случайно отражает картины по горизонтали для разнообразия.
        transforms.RandomHorizontalFlip(),
        # Третья "линза": превращает картину из объекта Pillow в тензор PyTorch.
        transforms.ToTensor(),
...
# Мы создаем функцию-помощника, которая будет применять этот конвейер к каждой картине.
def apply_transforms(examples):
    # Мы проходимся по каждой картине в порции (`batch`) и применяем к ней конвейер.
...
# Мы призываем "Проявителя" (VAE), который переводит образы в латентное пространство.
...
for epoch in range(num_train_epochs):
    ...
    for step, batch in tqdm(
        ...
        with torch.no_grad():
            # Шаг 1: Мы переносим чистую картину в "латентное пространство" с помощью VAE.
        ...
        # Шаг 5: Мы считаем ошибку (насколько предсказанный шум отличается от реального).
        loss = F.mse_loss(noise_pred, noise)
        # Шаг 6: Мы вычисляем "направления для исправления" для нашего "блокнота".
        loss.backward()
