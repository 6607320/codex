...
Этот пергамент — "экзамен" для нашей "Стилевой Печати". Его главная цель
(МАКРО-контекст) — продемонстрировать, как применять (инференс) обученный
LoRA-адаптер для генерации изображений.

Ритуал состоит из двух актов:
1.  **"Просветление":** Мы призываем "чистого" Духа-Демиурга и "надеваем"
    на его "сердце" (`UNet`) нашу обученную "магическую броню" (LoRA).
2.  **"Творение":** Мы даем "просветленному" Демиургу новое заклинание,
    содержащее "слово-активатор", и приказываем сотворить образ,
    которого не было в "учебнике", чтобы проверить, усвоил ли он сам СТИЛЬ.

Этот квест закрепляет понимание модульной природы LoRA и является основой
для создания коммерческих генеративных сервисов.
"""
...
# Мы призываем чертеж `DiffusionPipeline` из гримуара `diffusers` для
# магии творения.
from diffusers import DiffusionPipeline

# Мы призываем специальный инструмент `PeftModel` для работы с
# "блокнотами" (нашей Печатью).
from peft import PeftModel

# --- Акт 2: Призыв "Чистого" Демиурга ---
# Второй акт: мы призываем базового Духа-Демиурга, которого будем
# "просветлять".
...
pipe = DiffusionPipeline.from_pretrained(
    ...
    model_id,
    # Мы используем руну экономии VRAM, загружая модель в "легком" 16-битном
    # формате.
    torch_dtype=torch.float16,
    # Мы отключаем "Цензора", так как он может конфликтовать с нашим новым
    # стилем.
    safety_checker=None,
)
...
# Теперь, когда Демиург "просветлен", мы перемещаем весь конвейер на
# Кристалл Маны.
pipe = pipe.to("cuda")
...
# Четвертый, кульминационный акт: мы даем "просветленному" Демиургу новое
# задание.

# Мы создаем новое текстовое заклинание (промпт).
# Мы просим нарисовать то,
#  чего не было в нашем "учебнике" (астронавта), чтобы проверить,
# переносится ли сам СТИЛЬ. ВАЖНО: мы добавляем наше "магическое слово-активатор"
#  - 'in sks style'.
...
with torch.no_grad():
    ...
    image = pipe(
        ...
        prompt,
        # Мы указываем, что ритуал должен состоять из 50 шагов "очистки от
        # шума".
