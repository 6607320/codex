...
# Мы призываем помощника `numpy` для математических операций, особенно для матриц трансформации.
import numpy as np
...
# Мы загружаем наш артефакт 'magical_cat.png', который будет "актером" в обоих "фильмах".
...
for i in range(30):
    ...
    h, w, _ = zoomed.shape
    # Мы вычисляем координаты, чтобы вырезать центральную часть размером с исходный кадр.
