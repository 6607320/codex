# === quest_8_1.py ===
# Имя этого пергамента, знаменующего наш первый шаг в Сагу о Видеомагии.
...
# Объяснение, почему эта магия так важна для Техноманта.
...
# Мы даем имя "ящику" (папке), куда будем складывать наши "прореженные" карты-кадры.
output_folder = "sampled_frames"
# Мы произносим заклинание `makedirs`, чтобы создать эту папку, если ее еще не существует.
os.makedirs(output_folder, exist_ok=True)

# Мы устанавливаем "правило отбора": будем брать только каждый 3-й кадр.
# Это наш главный гиперпараметр, который мы можем менять для управления "плотностью" анализа.
frame_skip = 3

# --- Акт 3: Ритуал Прореживания ---
# Третий, кульминационный акт: мы пропускаем кинопленку через наш "магический фильтр".

# Мы оглашаем на кристалл (консоль) о начале ритуала.
print("Открываю кинопленку 'test_video.mp4' для прореживания...")
# Мы "захватываем" видеофайл, создавая "объект-проигрыватель" для чтения кадров.
...
while True:
    ...
    # Если заклинание `read` вернуло `success=False`, значит, кадры закончились, и мы прерываем ритуал.
    if not success:
