...
# Мы призываем `numpy` — магический калькулятор, незаменимый для работы с "сырой материей" данных.
import numpy as np

# Мы призываем `torch` — основу всей нашей техномагии, управляющую потоками маны.
import torch

# Мы призываем НОВЫЙ ГРИМУАР `datasets` — ключ, открывающий врата в Великую Библиотеку Hugging Face.
from datasets import load_dataset

# Мы призываем `scipy.io.wavfile` — заклинание для сохранения аудио-вибраций в материальный носитель (.wav).
...
def fetch_audio_from_hub(filename="command_from_hub.wav", sample_rate=16000):
    ...
    # Мы проводим ритуал Гармонизации: проверяем, совпадает ли "родная" частота с той, что понимает наш Голем.
    if original_sr != sample_rate:
        # Если нет, мы применяем заклинание `resample`, чтобы привести звук к нужной гармонике.
    ...
    # Мы произносим заклинание Материализации: Превращаем эфирный массив чисел в формат для записи.
...
def transcribe_audio(audio_path):
    ...
    # Мы с помощью librosa читаем наш локальный аудиофайл, убедившись, что его частота 16000 Гц.
    ...
    # Мы даем команду Голему: сгенерируй руны (ID токенов) на основе услышанного.
...
def classify_intent(text):
    ...
    # Мы создаем конвейер для классификации, сразу отправляя его на нужное устройство.
    classifier = pipeline(
...
def generate_response(intent):
    ...
    # Мы загружаем самого Голема 'distilgpt2' и отправляем его на Кристалл Маны.
    model = GPT2LMHeadModel.from_pretrained("distilgpt2").to(DEVICE)
    # Мы устанавливаем специальный токен для паддинга, чтобы избежать ошибок при генерации.
    tokenizer.pad_token = tokenizer.eos_token
    # Мы создаем "затравку" (начальный текст) для Голема в зависимости от намерения.
...
if __name__ == "__main__":
    ...
    # Шаг 4: Мы передаем определенное намерение Голему-Оракулу для генерации ответа.
