...
Этот пергамент превращает наш "одноразовый" конвейер големов из
предыдущего квеста в "живого", интерактивного ассистента. Его главная
цель (МАКРО-контекст) — освоить создание постоянно работающих приложений
и симуляцию сложных сценариев для надежной отладки.

Чтобы сделать ритуал на 100% воспроизводимым, мы используем "Театр
Марионеток":
1.  **Создание Реквизита:** С помощью гримуара `gTTS` мы программно
    создаем аудиофайлы для "реплик" пользователя.
2.  **Представление:** В "бесконечном" цикле мы "проигрываем" эти реплики
    одна за другой, прогоняя их через наш конвейер.

Этот подход, по сути, является **end-to-end регрессионным тестом**,
доказывая мастерство инженера в создании надежных, автоматизированных
тестовых сценариев для сложных AI-систем.
"""
...
def prepare_dialogue_props(dialogue_script, lang="ru"):
    ...
    # Мы произносим заклинание из гримуара `os`: "Создай папку 'dialogue_audio',
    ...
    # `enumerate` — хитрый дух-счетчик, который дает нам и номер реплики (`i`),
    #  и ее текст (`text`).
...
def generate_response(intent):
    ...
    prompts = {
        ...
        "неизвестная команда": (
            "Я не совсем понял команду, " "попробуйте переформулировать: "
        ),
    }
...
if __name__ == "__main__":
    ...
    for i, text_command in enumerate(dialogue_script):
        # Мы используем заклинание 'сна' из гримуара `time`,
        #  чтобы создать паузу в 2 секунды.
