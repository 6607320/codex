...
# Мы призываем НОВЫЙ ГРИМУАР `gtts` (Google Text-to-Speech) — духа-глашатая, способного превращать текст в речь.
...
def prepare_dialogue_props(dialogue_script, lang="ru"):
    ...
    for i, text in enumerate(dialogue_script):
        ...
        # Мы призываем духа-глашатая `gTTS`, передавая ему текст и язык ('ru' — русский).
...
if __name__ == "__main__":
    # Мы создаем наш "сценарий" — список команд, которые мы хотим протестировать по очереди.
    dialogue_script = [
        # Первая реплика.
        "Расскажи мне шутку",
        # Вторая реплика.
    ...
    # Шаг 1: Мы вызываем подготовительный ритуал, чтобы создать все аудиофайлы заранее.
    ...
    for i, text_command in enumerate(dialogue_script):
        ...
        # Шаг 2: Мы вызываем ритуал распознавания речи, передавая ему аудиофайл.
        command_text = transcribe_audio(audio_file)

        # Шаг 3: Мы проверяем, не содержит ли распознанный текст наше "слово-ключ".
