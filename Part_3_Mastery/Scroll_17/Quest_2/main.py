"""Квест 17.2: Зарядка Амулета (через Docker).

Этот пергамент — это "чертеж" нашего "Говорящего Амулета", предназначенный
для "запечатывания" в Docker-контейнер. Его главная цель (МАКРО-контекст) —
служить сердцем самодостаточного, переносимого артефакта.

В отличие от Квеста 17.1, где мы запускали этот скрипт напрямую, здесь он
является лишь "душой", которая будет помещена в "тело" (контейнер),
описанное в `Dockerfile`. Этот ритуал демонстрирует, как подготовить
AI-сервис к развертыванию в любой среде, от локальной машины до облачной
цитадели, достигая истинной воспроизводимости.
"""

# --- Акт 1: Подготовка Гримуаров ---
# Начинается первый акт: мы призываем все необходимые знания и инструменты.
# Мы призываем чертеж `FastAPI` для создания "порталов".
from fastapi import FastAPI

# Мы призываем чертеж `BaseModel` для описания данных, которые будут
# приходить в портал.
from pydantic import BaseModel

# Мы призываем наш старый добрый "амулет" `pipeline`.
