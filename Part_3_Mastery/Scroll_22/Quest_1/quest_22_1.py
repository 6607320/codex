"""Квест 22.1: Призыв Компаса.

Этот пергамент открывает "Свиток Магического Компаса". Его главная цель
(МАКРО-контекст) — освоить применение (inference) мультимодального голема
`CLIP`, который понимает и изображения, и текст в едином "пространстве
смыслов".

Мы даем Голему изображение и несколько текстовых описаний и просим его
определить, какое описание лучше всего соответствует картинке. Ключевая
магия ритуала — использование `CLIPProcessor` для одновременной подготовки
текста и изображения, и `CLIPModel` для получения "сырой" оценки схожести
(`logits_per_image`), которую мы затем превращаем в вероятности. Этот квест
также служит уроком о **"сдвиге домена"**.
"""
...
if __name__ == "__main__":
    ...
    # Призываем самого Голема CLIP по его имени и отправляем на магический
    # алтарь.
    ...
    # 'streaming=True' - мудрый трюк, чтобы не качать всю "книгу",
    #  а брать данные по частям, как из потока.
    ds = load_dataset("cifar100", split="train", streaming=True)

    # Мы должны найти в потоке картинку с нужной нам меткой.
    # Задаем "точную метку" (fine_label) для класса 'cat', которая в cifar100
    # равна 20.
    target_label = 20
    # Создаем пустой "сосуд" для хранения нашего изображения.
    cat_image = None
    # 'for example in ds:' - это заклинание, которое перебирает примеры
    #  (картинка + метка) из потока данных.
    for example in ds:
        # 'if example['fine_label'] == target_label:' - проверяем,
        # является ли "точная метка" текущей картинки той, что мы ищем.
        if example["fine_label"] == target_label:
            # Если да, извлекаем само изображение, которое хранится под ключом
            # 'img'.
    ...
    if cat_image is None:
        # 'raise ValueError(...)' - заклинание "поднять тревогу",
        #  которое аварийно завершит ритуал с сообщением.
        raise ValueError("Не удалось найти изображение кота в датасете!")

    # 'image.convert("RGB")' - важное заклинание-предохранитель.
    #  Оно гарантирует, что у картинки 3 цветовых канала.
    ...
    # 'padding=True' - руна, говорящая "если тексты разной длины,
    #  дополни короткие специальными токенами".
    # '.to(DEVICE)' - немедленно отправляем подготовленные данные
    #  на наш магический алтарь.
    inputs = processor(
        text=text_descriptions, images=image, return_tensors="pt", padding=True
    ).to(DEVICE)

    # Отправляем подготовленные данные в "разум" Голема CLIP.
    # '**inputs' - магическая руна "распаковки",
    #  которая передает содержимое словаря 'inputs' как именованные аргументы.
    outputs = model(**inputs)
    # Из вывода голема мы извлекаем 'logits_per_image' - это "сырая" оценка
    # схожести.
    logits_per_image = outputs.logits_per_image
    # Применяем заклинание 'softmax',
    #  чтобы превратить сырые оценки в понятные вероятности (от 0 до 1).
    # 'dim=1' указывает, по какому измерению применять softmax
    #  (вдоль списка наших гипотез).
    probs = logits_per_image.softmax(dim=1)

    # --- Акт 4: Оглашение Вердикта ---
    # Сообщаем о результате.
    print("\n--- Вердикт Магического Компаса ---")
    # Извлекаем данные из тензора: '.cpu()' - вернуть на Разум,
    # '.detach()' - отвязать от графов вычислений, '.numpy()' - превратить в массив.
    # '[0]' - так как у нас была всего одна картинка, мы берем результаты для нее.
    probabilities = probs.cpu().detach().numpy()[0]

    # Проходим по каждой гипотезе и ее вероятности одновременно с помощью духа
    # 'zip'.
