...
Этот пергамент демонстрирует применение `CLIP` как "Магического Компаса" для
решения практической задачи семантического поиска. Его главная цель
(МАКРО-контекст) — освоить фундаментальный паттерн "индексация-поиск".

Ритуал состоит из двух актов:
1.  **Индексация:** Мы создаем небольшую "галерею" изображений и заранее
    вычисляем их "ауры" (эмбеддинги) с помощью `CLIP`.
2.  **Поиск:** Мы даем Компасу текстовый запрос, вычисляем его "ауру" и
    находим то изображение из галереи, чья "аура" ему наиболее "созвучна"
    (имеет наибольшее скалярное произведение).

Этот квест также закрепляет урок о "сдвиге домена", показывая, как `CLIP`
может находить сходство на уровне абстрактных форм, а не объектов.
"""
...
if __name__ == "__main__":
    ...
    # Создаем "сосуд", где будем хранить найденные артефакты (картинки и их
    # истинные имена).
    gallery = {}
    ...
    # '.get_image_features' - специальное заклинание CLIP
    #  для получения только аур изображений.
    ...
    # Главное заклинание поиска: вычисляем "резонанс"
    #  между аурой текста и КАЖДОЙ аурой из нашей галереи.
    # '@' - это руна матричного умножения,
    #  самый эффективный способ посчитать все резонансы разом.
    ...
    # 'torch.argmax(similarities)' - заклинание,
    #  которое находит ИНДЕКС самого большого резонанса.
