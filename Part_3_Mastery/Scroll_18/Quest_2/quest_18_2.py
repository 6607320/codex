...
Этот пергамент демонстрирует переход от простого "видения" к **анализу
увиденного**. Его главная цель (МАКРО-контекст) — научить нашу "Сторожевую
Химеру" не просто рисовать рамки, а **считать** количество объектов
определенного типа в каждом кадре.

Ключевой особенностью этого ритуала является добавление **простейшей
"памяти"** (`last_known_results`). Она решает проблему "мерцающей" детекции,
когда Химера на мгновение теряет объект из виду. Если на текущем кадре
ничего не найдено, мы "вспоминаем" и рисуем результат с предыдущего удачного
кадра. Этот квест — первый шаг к созданию более стабильных и надежных
систем видеоаналитики.
"""
...
# Начинается четвертый, кульминационный акт:
#  мы начинаем охоту с использованием "памяти".
...
for _ in tqdm(range(total_frames), desc="Подсчет на кинопленке"):
    ...
    # `... and last_known_results is not None`:
    #  "...И (`and`) при этом у нас есть что-то в 'памяти'?"
    if len(results.xyxy[0]) == 0 and last_known_results is not None:
        # `detections_to_draw = last_known_results`:
        #  "...то для рисования мы будем использовать 'воспоминание'".
        detections_to_draw = last_known_results
    # В противном случае (если Химера что-то нашла)...
        ...
        if len(detections_to_draw) > 0:
            ...
            #  "...мы ОБНОВЛЯЕМ нашу 'память'".
            last_known_results = detections_to_draw

    # --- Ритуал Подсчета и Рисования ---
    # Мы готовим счетчик. Важно: мы обнуляем его для КАЖДОГО нового кадра.
    object_counter = 0
    # Мы определяем нашу цель для подсчета, основанную на результатах
    # диагностики.
    target_class = "stop sign"

    # `if detections_to_draw is not None`: Мы используем защитную руну на случай,
    # если и свежих, и старых результатов нет.
    if detections_to_draw is not None:
        ...
        for *box, conf, cls in detections_to_draw:
            ...
            # `x1, y1, x2, y2 = map(int, box)`: Мы "распаковываем"
            #  и округляем координаты рамки.
...
print(
    f"\nРитуал завершен. {frame_count} кадра с подсчетом "
    f"сохранено в папку '{output_folder}'."
)
