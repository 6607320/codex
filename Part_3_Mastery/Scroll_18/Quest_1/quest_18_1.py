...
Этот пергамент открывает "Свиток Сторожевой Химеры". Его главная цель
(МАКРО-контекст) — пробудить "духа-стража" YOLOv5n и научить его "охотиться"
на объекты в потоке движущихся образов (видеофайле).

Мы осваиваем не просто "использование" модели, а **работу с ее результатами
на низком уровне**. Ритуал извлекает из "отчета" Химеры координаты (`box`),
уверенность (`conf`) и класс (`cls`) найденных объектов. Затем, с помощью
магии `OpenCV`, мы **сами рисуем** эти результаты (ограничивающие рамки
и подписи) на каждом кадре, превращая "сырые данные" в наглядный
визуальный отчет.
"""
...
for _ in tqdm(range(total_frames), desc="Охота на кинопленке"):
    ...
    # --- Ритуал "Явных Чернил" ---
    # `results.xyxy[0]` — это "отчет" Химеры.
    #  Мы перебираем каждую "строку" (найденный объект) в этом отчете.
    for *box, conf, cls in results.xyxy[0]:
        # `model.names[int(cls)]` — мы заглядываем в "паспорт" Химеры,
        #  чтобы узнать имя класса по его номеру.
...
cap.release()
# Мы оглашаем, что ритуал завершен, и сообщаем, сколько "трофеев" было
# сохранено.
print(
    f"\nРитуал завершен. {frame_count} помеченных кадров "
