...
Этот пергамент демонстрирует создание нашего первого Голема-Оракула,
способного заглядывать в будущее. Его главная цель (МАКРО-контекст) —
освоить полный цикл создания предсказательной модели на основе LSTM.

В сердце Голема мы помещаем "магический кристалл памяти" — слой `nn.LSTM`,
который позволяет ему помнить прошлое, чтобы предсказывать будущее. Ритуал
состоит из всех канонических этапов:
1.  Подготовка "летописи времени" (нормализация, создание последовательностей).
2.  Проектирование архитектуры Оракула (`OracleLSTM`).
3.  Его долгое Наставление в "ручном" режиме.
4.  "Экзамен" на данных, которые он никогда не видел, с визуализацией результата.

Этот квест наглядно показывает и силу LSTM (улавливание сезонности), и ее
фундаментальную слабость (неспособность к экстраполяции тренда).
"""
...
# Призываем гримуар 'pandas' (pd) - для работы с "летописями времени" в
# табличной форме.
import pandas as pd
...
# Эта функция-ритуал превращает плоскую летопись в набор "карточек" для
# обучения.
def create_sequences(data, seq_length):
    # Создаем пустой "мешок" (список) для хранения последовательностей
    # прошлого.
    sequences = []
    # Создаем пустой "мешок" (список) для хранения соответствующих значений
    # будущего.
    labels = []
    # Запускаем цикл, который будет двигать "магическое окно" по нашей
    # летописи.
    for i in range(len(data) - seq_length):
        ...
        labels.append(label)
    # Превращаем наши "мешки" в удобные для вычислений массивы numpy и
    # возвращаем их.
...
class OracleLSTM(nn.Module):
    ...
    def __init__(self, input_size=1, hidden_layer_size=100, output_size=1):
        # 'super().__init__()' - обязательное заклинание,
        #  призывающее дух предка для корректной инициализации.
        super().__init__()
        # 'self.hidden_layer_size' - задаем размер "разума" голема,
        #  сколько информации он хранит в своей памяти.
        self.hidden_layer_size = hidden_layer_size

        # 'self.lstm' - создаем главный компонент,
        #  LSTM-слой, который будет обрабатывать последовательности.
        self.lstm = nn.LSTM(input_size, hidden_layer_size)

        # 'self.linear' - создаем простой линейный слой,
        #  который превратит "мысли" LSTM в финальное предсказание.
    ...
    def forward(self, input_seq):
        # Извлекаем сосуд с Краткосрочной Памятью и отправляем на магический
        # алтарь.
        h0 = self.hidden_cell[0].to(input_seq.device)
        # Извлекаем сосуд с Долгосрочной Памятью и отправляем на магический
        # алтарь.
        ...
        #  'view' меняет форму тензора для совместимости.
...
if __name__ == "__main__":
    ...
    # Извлекаем только колонку 'value', превращая ее в массив numpy для
    # вычислений.
    ...
    # Вызываем наш ритуал для создания "учебных карточек" из нормализованных
    # данных.
    ...
    for i in range(epochs):
        ...
        # 'zip' - дух, который позволяет идти по двум "мешкам"
        #  (X_train, y_train) одновременно.
        for seq, labels in zip(X_train, y_train):
            # Отправляем текущую "карточку" (прошлое и будущее) на магический
            # алтарь.
            seq, labels = seq.to(DEVICE), labels.to(DEVICE)

            # 'optimizer.zero_grad()' - важнейшее заклинание:
            #  "Очисти память наставника от прошлых ошибок".
            optimizer.zero_grad()
            # Создаем пустой "сосуд" для Краткосрочной Памяти и отправляем на
            # алтарь.
            h0 = torch.zeros(1, 1, model.hidden_layer_size).to(DEVICE)
            # Создаем пустой "сосуд" для Долгосрочной Памяти и отправляем на
            # алтарь.
            ...
            # 'y_pred = model(seq)' - Оракул смотрит на прошлое ('seq')
            #  и делает предсказание.
            y_pred = model(seq)

            # 'single_loss = ...' - Сравниваем предсказание ('y_pred')
            #  с истинным будущим ('labels').
            single_loss = loss_function(y_pred, labels)
            # 'single_loss.backward()' - Вычисляем,
            #  в какую сторону нужно исправить "разум" голема.
            single_loss.backward()
            # 'optimizer.step()' - Наставник делает шаг,
            #  исправляя "разум" (веса) голема.
            optimizer.step()
    ...
    #  (отключаем случайности вроде dropout).
    model.eval()

    # Запускаем цикл предсказаний на столько шагов, сколько у нас дней в
    # "экзаменационной" части.
    for i in range(len(test_data)):
        # Берем последнюю известную нам последовательность и превращаем ее в
        # тензор.
        seq = torch.FloatTensor(test_inputs[-seq_length:]).to(DEVICE)

        # 'with torch.no_grad():' - создаем "поле тишины",
        #  где мы не считаем ошибки, только предсказываем.
        with torch.no_grad():
            # Создаем пустой "сосуд" для Краткосрочной Памяти (h_n) на нужном
            # устройстве.
            h_n = torch.zeros(1, 1, model.hidden_layer_size).to(DEVICE)
            # Создаем пустой "сосуд" для Долгосрочной Памяти (c_n) на нужном
            # устройстве.
            c_n = torch.zeros(1, 1, model.hidden_layer_size).to(DEVICE)
            # **ИСПРАВЛЕНИЕ ОШИБКИ**: Присваиваем "чистую" память голему
            #  перед каждым предсказанием.
    ...
    # 'plt.axvline(...)' - приказываем нарисовать вертикальную
    #  красную линию в точке начала экзамена.
