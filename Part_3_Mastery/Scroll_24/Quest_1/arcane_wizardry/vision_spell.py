"""Квест 24.1: "Заклинание Зрения".

Этот пергамент — это "душа", которая будет запечатана внутри нашего
первого `pip`-пакета. Его главная цель (МАКРО-контекст) — служить
примером простого, но рабочего Python-модуля, который можно
импортировать и использовать после установки пакета.

Функционально этот скрипт является копией Квеста 3.1 ("Призыв Всевидящего
Ока"). Однако его **имя** (`vision_spell.py`) и **расположение** (внутри
папки `arcane_wizardry/` с `__init__.py`) демонстрируют каноническую
структуру Python-пакета, готовую к "запечатыванию".
"""
...
# Внутри 'sample' находится словарь. Сам образ (объект Pillow) лежит под
# ключом 'img'.
...
# классов с их "уверенностью" (score).
#  Мы можем сравнить этот вердикт с "истинной сутью",
