...
# Квест 22.1: Призыв Компаса (Финальная Архивная версия)
...
if __name__ == "__main__":
    ...
    # 'cat' в cifar100 имеет метку (label) 20.
    target_label = 20
    ...
    for example in ds:
        # 'if example['label'] == target_label:' - проверяем, является ли метка текущей картинки той, что мы ищем.
        if example["label"] == target_label:
            # Если да, извлекаем само изображение из примера.
            cat_image = example["image"]
            # 'break' - заклинание "прерви поиск", так как мы нашли то, что искали.
            break

    # Проверка-предохранитель на случай, если что-то пошло не так.
