...
## Инструменты Диагностики (Аварийный набор)

В арсенале Мастера есть три особых инструмента для проверки здоровья системы индексации. Используйте их, если Оракул кажется "слепым", "запутавшимся во времени" или возникают сомнения в полноте данных.

### 1. `debug_rag_test.py` (Диагност)

**Цель:** Проверка **зрения**. Эмулирует процесс сканирования без записи изменений и без порчи манифеста.
**Когда использовать:**

- Если `rag_builder.py` сообщает, что не нашел новых файлов, хотя они есть.
- Для проверки правил исключения (убедиться, что скрипт **не видит** секретные файлы `.env`).
  **Запуск:** `python debug_rag_test.py`

### 2. `audit_index.py` (Аудитор)

**Цель:** Проверка **целостности**. Сравнивает реальность (файлы на диске) с летописью (`manifest.json`), чтобы найти пропущенные файлы.
**Когда использовать:**

- Если кажется, что важный файл (например, новая статья `.md`) не попал в PDF.
- После рефакторинга структуры папок проекта.
  **Запуск:** `python audit_index.py`

### 3. `find_paradox.py` (Детектив Времени)

**Цель:** Проверка **времени**. Ищет файлы, которые существуют в нескольких версиях в разных PDF-томах.
**Когда использовать:**

- Если Оракул дает противоречивые или устаревшие ответы по какому-то конкретному файлу.
- Для поиска "идеального кандидата" для проверки способности Оракула разрешать временные конфликты.
  **Запуск:** `python find_paradox.py`
