...
В арсенале Мастера есть два особых инструмента для проверки здоровья системы индексации. Используйте их, если Оракул кажется "слепым" или возникают сомнения в полноте данных.

### 1. `audit_index.py` (Аудитор)

**Цель:** Проверка целостности. Сравнивает реальность (файлы на диске) с летописью (`manifest.json`).
**Когда использовать:**
