"""Инструмент: Строитель "Живой Летописи" для `Оракула-Архивариуса`.

Этот пергамент — не учебный Квест, а **"инструмент Мастера"**. Его главная
цель (МАКРО-контекст) — автоматически сканировать "Великий Кодекс Техномагии",
находить все текстовые артефакты (`.py`, `.md`, `.yml` и др.), и собирать
их содержимое в серию PDF-томов (`notebooklm_sources/`).

Эти PDF-тома служат **источником знаний** для нашего `Оракула-Архивариуса`
(RAG-системы на базе NotebookLM), превращая его в безупречную, всегда
актуальную "живую" память о Кодексе. Ритуал работает инкрементально: он
отслеживает изменения в файлах с помощью "печатей" (хешей) в `manifest.json`
и пересобирает только тот том, который был затронут изменениями.
"""
...
PROJECT_DIR = "../../"
# Устанавливаем Путь для Артефактов (текущая папка инструмента)
ARTEFACTS_DIR = "."
...
def save_manifest(manifest_path: str, data: Dict[str, Any]):
    # Собираем канонический путь к Летописи (в папке инструмента).
...
def create_pdf_from_text(text_content: str, output_path: str):
    ...
    try:
        ...
        # Произносим заклинание, очищающее текст от запретных рун.
        text_content = text_content.translate(translation_table)

        # Призываем в мир новую, пустую сущность PDF-свитка.
        pdf = FPDF()
        # Обучаем Духа-Каллиграфа единственному руническому шрифту.
        ...
        pdf.add_page()
        # Вписываем очищенное текстовое содержимое на лист.
...
def main():
    ...
    if manifest_data["chunks"]:
        ...
        if last_chunk["file"] == LIVING_CHUNK_FILENAME:
            ...
            for source_file in last_chunk["sources"]:
                ...
                # Призываем дух ОС, дабы проверить,
                # существует ли материальная плоть свитка на диске.
                if os.path.exists(source_file):
                    # Если свиток реален, бросаем его имя в магический котел для
                    # новой переплавки в PDF.
                    files_to_reprocess.add(source_file)
                # Если же свиток обратился в прах и
                # более не существует в подлунном мире...
                else:
                    # ...мы извлекаем память о его печати из Летописи,
                    # дабы не множить призраков.
                    manifest_data["files"].pop(source_file, None)
    ...
    # Финальный щит: оставляем только те пути, которые реально являются файлами.
    sorted_files_to_process = [f for f in sorted_files_to_process if os.path.isfile(f)]
    ...
    for filepath in sorted_files_to_process:
        ...
        try:
            ...
            with open(filepath, "r", encoding="utf-8", errors="ignore") as f:
                ...
                content = f.read()
        # Если ритуал чтения (или получения времени) был прерван ошибкой...
        ...
        # Создаем магический заголовок с Печатью Времени.
        file_header = f"\n\n--- НАЧАЛО: {filepath} (timestamp: {timestamp}) ---\n\n"
        # Создаем магическое завершение.
        file_footer = f"\n\n--- КОНЕЦ: {filepath} ---\n\n"
