...
def calculate_sha256(filepath: str) -> str:
    ...
    try:
        ...
        with open(filepath, "rb") as f:
            # Читаем свиток по частям (блоками по 4096 байт), чтобы не перегружать память.
...
def scan_project_files(
    ...
    for root, dirs, files in os.walk(directory, topdown=True):
        # Изгоняем папки из Черного Списка из списка `dirs` на лету, чтобы `os.walk` в них не заходил.
        ...
        for file in files:
            ...
            if file.endswith(extensions) or file in filenames:
                # Соединяем путь к папке (`root`) и имя свитка (`file`), создавая полный путь.
    ...
    try:
        ...
        # Изгоняем путь к самому себе из списка, ибо ритуал не должен анализировать сам себя.
...
def main():
    ...
    if manifest_data["chunks"]:
        ...
        if last_chunk["file"] == LIVING_CHUNK_FILENAME:
            ...
            # Начинаем цикл по всем свиткам, которые были запечатаны в уничтоженном томе.
    ...
    # Превращаем наш котел (множество) в отсортированный список для предсказуемого порядка.
