# Призываем духа операционной системы `os` для взаимодействия с файлами и
# путями.
import os

# Призываем духа криптографии `hashlib` для сотворения нерушимых печатей
# (хешей).
import hashlib
...
# Из великой библиотеки `fpdf` призываем сущность `FPDF`, нашего
# Духа-Каллиграфа.
from fpdf import FPDF

# Призываем сущности из `typing` для наложения на ритуалы рун ясности
# (аннотаций типов).
...
# Определяем Истинные Имена особых свитков, что не имеют расширений, но
# также важны.
...
def calculate_sha256(filepath: str) -> str:
    ...
    try:
        # Открываем свиток в бинарном режиме (`"rb"`) для чтения его плоти как
        # есть.
        with open(filepath, "rb") as f:
            # Читаем свиток по частям (блоками по 4096 байт), чтобы не перегружать
            # память.
            for byte_block in iter(lambda: f.read(4096), b""):
                # Каждую прочитанную часть бросаем в котел, постепенно формируя
                # печать.
...
# Определяем ритуал `scan_project_files` для проведения Великого Обхода
# Кодекса.
def scan_project_files(
    directory: str, extensions: tuple, filenames: tuple, exclude_dirs: set
) -> List[str]:
    # Готовим пустой магический мешок для сбора путей ко всем достойным
    # свиткам.
    all_files = []
    ...
    try:
        # Определяем абсолютный путь к самому этому пергаменту
        # (`rag_builder.py`).
    ...
    # Если ритуал запущен в среде, где `__file__` не определен...
        ...
        pass
    # Возвращаем отсортированный по алфавиту список путей для порядка и
    # предсказуемости.
...
def load_manifest(manifest_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        with open(manifest_path, "r", encoding="utf-8") as f:
            # Преобразуем текст свитка из JSON в структуру данных и возвращаем
            # её.
            return json.load(f)
...
def create_pdf_from_text(text_content: str, output_path: str):
    ...
    try:
        ...
        pdf.add_page()
        # Вписываем все текстовое содержимое на лист, с автоматическим
        # переносом строк.
        pdf.multi_cell(0, 5, text_content)
        # Убеждаемся, что существует папка, в которую мы хотим сохранить наш
        # том.
...
def main():
    ...
    # Собираем полный путь к Летописи, соединяя корень Кодекса и имя файла
    # Летописи.
    ...
    # Убеждаемся, что Хранилище Томов существует, создавая его при
    # необходимости.
    os.makedirs(output_dir_path, exist_ok=True)
    # Проводим ритуал чтения Летописи, чтобы узнать о предыдущем состоянии
    # Кодекса.
    manifest_data = load_manifest(manifest_path)
    # Объявляем о начале фазы инвентаризации.
    print("1. Сканирование файлов проекта...")
    # Запускаем Великий Обход Кодекса, чтобы получить список всех релевантных
    # свитков.
    ...
    # Создаем список свитков, которые либо являются новыми, либо их печать
    # изменилась.
    ...
    # Готовим котел для переплавки, помещая в него все новые и измененные
    # свитки.
    ...
    if manifest_data["chunks"]:
        ...
        if last_chunk["file"] == LIVING_CHUNK_FILENAME:
            ...
            for source_file in last_chunk["sources"]:
                # Изгоняем из Летописи в памяти запись о каждом из этих
                # свитков.
    ...
    for filepath in sorted_files_to_process:
        ...
        # Если текущий том не пуст и добавление нового свитка превысит Лимит
        # Слов...
        if (
            current_chunk_word_count > 0
            ...
            manifest_data["chunks"].append(
                {
                    # Записываем его имя, количество слов и список
                    # свитков-источников.
                    "file": part_filename,
        ...
        file_footer = f"\n\n--- КОНЕЦ: {filepath} ---\n\n"
        # Добавляем содержимое свитка, обрамленное метаданными, в астральный
        # сосуд.
