...
def parse_codex():
    ...
    if not os.path.exists(codex_path): return [], ""
    with open(codex_path, 'r', encoding='utf-8') as f: lines = f.readlines()
    ...
    current_part_num, current_part_title, current_scroll_title = "0", "Неизвестная Часть", "Неизвестный Свиток"
    ...
    for line in lines:
        ...
        if part_match:
            ...
            current_part_title = f"Часть {current_part_num}: {part_match.group(2).strip()}"
            continue
        ...
        if quest_match:
            if current_quest: quests.append(current_quest)
            q_id = quest_match.group(1)
            q_title = re.sub(r"\s*`?\[.*?\]`?$", "", quest_match.group(2)).strip()
            current_quest = {
                "id": q_id, "title": q_title, "partNumber": current_part_num,
                "partTitle": current_part_title, "scrollTitle": current_scroll_title,
                "legend_md": "", "legend_html": "", "manifest_html": "<em>Свиток не найден...</em>",
                "status": "locked"
            }
        ...
        if current_quest:
            if "<!--" in line: capture_comment = True
            if capture_comment: comment_buffer.append(line)
...
def build():
    ...
    if not os.path.exists(QUESTS_DIR): os.makedirs(QUESTS_DIR)
    if not os.path.exists(SCENARIOS_DIR): os.makedirs(SCENARIOS_DIR)
    ...
    for root, _, _ in os.walk(PROJECT_ROOT):
        if any(d in root for d in ["site_builder", ".git", ".dvc"]): continue
    ...
    for quest in quests_list:
        ...
        if quest_folder:
            ...
            if os.path.exists(manifest_path):
                with open(manifest_path, 'r', encoding='utf-8') as f: manifest_content = f.read()
                quest['manifest_html'] = markdown.markdown(textwrap.dedent(manifest_content), extensions=['fenced_code', 'tables'])
                quest['status'] = 'active'
        
        scenario_file = os.path.join(SCENARIOS_DIR, f"quest_{q_id}.json")
        if os.path.exists(scenario_file):
            with open(scenario_file, 'r', encoding='utf-8') as f: scenario_data = json.load(f)
        
        data_to_hash = quest['title'] + quest['legend_md'] + manifest_content + json.dumps(scenario_data)
        ...
        if cache.get(q_id) != current_hash or not os.path.exists(json_path):
            full_quest_data = {
                ...
                "legend": quest['legend_html'], "manifest": quest['manifest_html'],
                "status": quest['status'], "scenario": scenario_data
            }
            with open(json_path, 'w', encoding='utf-8') as f: json.dump(full_quest_data, f, ensure_ascii=False, indent=2)
            updated_count += 1
        
        index_data.append({
            "id": quest['id'], "title": quest['title'], "partNumber": quest['partNumber'],
            "partTitle": quest['partTitle'], "scrollTitle": quest['scrollTitle'], "status": quest['status']
        })

    final_index = {"readme": readme_html, "quests": index_data}
    with open(INDEX_FILE, 'w', encoding='utf-8') as f: json.dump(final_index, f, ensure_ascii=False, indent=2)
