...
# Мы призываем НОВЫЙ гримуар, хранящий духа-"Инструментатора", который встроит в наш Амулет "магические глаза".
from prometheus_fastapi_instrumentator import Instrumentator

# Мы призываем гримуар, хранящий "чертеж" для описания данных, которые будут приходить в портал.
from pydantic import BaseModel

# Мы призываем наш старый добрый "амулет" `pipeline` из великой библиотеки `transformers`.
...
class TextInput(BaseModel):
    # Мы объявляем, что каждое 'послание' должно содержать поле `text`, и его тип должен быть `str` (строка).
    text: str
...
# Мы пробуждаем "Инструментатора" и приказываем ему "оснастить" наш портал, а затем "открыть" врата для наблюдения.
...
@app.post("/analyze")
# Мы определяем заклинание, которое будет исполняться, когда кто-то стучится в эти 'врата'.
