"""Квест 32.2: "Камень Истины" (валидационный набор данных).

Этот пергамент не является исполняемым ритуалом. Его единственная цель
(МАКРО-контекст) — хранить "Камень Истины" (`VALIDATION_SET`).

Этот священный эталон используется главным пергаментом `main.py`
для проведения "ритуала самопроверки". Он является неотъемлемой частью
артефакта, который будет запечатан в Docker-контейнер и протестирован
нашим "Стражем-Архитектором" в CI/CD пайплайне, доказывая, что наш
финальный Голем способен к самодиагностике и его работа может быть
автоматически верифицирована.
"""
