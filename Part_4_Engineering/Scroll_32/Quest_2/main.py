"""Квест 32.2: Установка "Телеметрии" в Кузницу (Точка входа).

Этот пергамент — "душа" финального артефакта, который будет собран
"Стражем-Архитектором". Его главная цель (МАКРО-контекст) — служить
"подопытным" для нашего самого сложного CI/CD пайплайна, который включает
в себя не только сборку, но и **сбор "улик" (артефактов)**.

Функционально этот скрипт — это наш "Амулет с Развилкой" из Свитка 30.
Его истинное предназначение — быть частью ритуала, где "Страж-Архитектор"
сначала запускает тесты (`pytest`), генерирует "отчет о покрытии", а затем
использует заклинание `actions/upload-artifact`, чтобы извлечь этот отчет
из "черного ящика" и сохранить его для нас.

Этот квест демонстрирует искусство "криминалистики MLOps" — умение
настроить автоматические механизмы так, чтобы они не просто умирали молча,
а собирали и оставляли детальные улики для отладки.
"""
