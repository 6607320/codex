"""Квест 29.2: Ритуал "Тревожного Сигнала".

Этот пергамент наделяет наш "Говорящий Амулет" совестью. Его главная цель
(МАКРО-контекст) — перейти от простого технического мониторинга ("жив ли
сервис?") к **мониторингу качества AI-модели** ("правильно ли он работает?").

Ритуал состоит из двух актов:
1.  **Сотворение "Тревожного Колокола":** Мы создаем кастомную метрику
    `Prometheus` типа `Gauge`, которой даем имя `model_accuracy`.
2.  **Ритуал "Самопроверки":** Мы создаем новые "врата" (`/validate`), при
    обращении к которым наш Амулет проводит "экзамен": прогоняет себя
    через "Кристалл Истины" (валидационный набор данных) и обновляет
    значение "Тревожного Колокола" текущей точностью.

Этот квест доказывает, как превратить AI из "черного ящика" в предсказуемый
и управляемый бизнес-инструмент.
"""
...
# Мы призываем гримуар `Pydantic` для создания "чертежей" для магических
# посланий.
...
# Мы призываем духа-"Инструментатора", который наделяет наш Амулет
# самосознанием.
from prometheus_fastapi_instrumentator import Instrumentator

# Мы призываем чертеж для "Тревожного Колокола" (`Gauge`) — особой
# метрики, которая может показывать уровень.
from prometheus_client import Gauge

# Мы призываем наш "Кристалл Истины" (`VALIDATION_SET`) — священный набор
# эталонных свитков для проверки.
from validation_data import VALIDATION_SET

# --- Акт 2: Подготовка "Духа" внутри Портала ---
# Начинается второй акт: мы призываем "Духа",
#  который будет жить внутри нашего "Амулета".
...
# Начинается четвертый акт: мы сотворяем сам "Портал" и наш новый
...
app = FastAPI(
    ...
    description=(
        "Магический портал, который определяет эмоции " "и следит за своей точностью."
    ),
...
ACCURACY_GAUGE = Gauge(
    # Мы даем ему имя 'model_accuracy', которое будет видно в летописях
    # Prometheus.
    "model_accuracy",
...
def validate_model():
    ...
    # Мы подсчитываем, сколько всего "эталонных свитков" в нашем "Кристалле
    # Истины".
    ...
    for item in VALIDATION_SET:
        ...
        # Мы извлекаем из предсказания только метку ('POSITIVE' или
        # 'NEGATIVE').
        predicted_label = prediction["label"]

        # Мы сверяемся с "Кристаллом Истины": совпадает ли предсказание с
        # истиной?
    ...
    # Мы заставляем наш "Тревожный Колокол" звонить с новым значением,
    # показывая текущую точность.
