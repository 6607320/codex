"""Квест 29.2: "Кристалл Истины" (валидационный набор данных).

Этот пергамент не является исполняемым ритуалом. Его единственная цель
(МАКРО-контекст) — хранить "Кристалл Истины" (`VALIDATION_SET`).
...
"истинной сутью" (метками). Главный пергамент `main.py` будет использовать
этот "Кристалл" для проведения "ритуала самопроверки", чтобы измерить
текущую точность "Говорящего Амулета".
"""
