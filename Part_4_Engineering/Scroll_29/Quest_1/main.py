"""Квест 29.1: Создание "Сторожевой Башни" (Точка входа).

Этот пергамент — "душа" нашего Говорящего Амулета, но с дарованным ему
"зрением". Его главная цель (МАКРО-контекст) — продемонстрировать, как
встроить в FastAPI-приложение магию "наблюдаемости" (observability).

Ключевой элемент этого ритуала — призыв "Инструментатора" из гримуара
`prometheus_fastapi_instrumentator`. Это могущественный дух, который
автоматически "оснащает" наш портал, создавая "врата" `/metrics`. Через
эти врата "Неусыпный Страж" (`Prometheus`) сможет в реальном времени
"считывать" состояние нашего Амулета (например, количество запросов,
их длительность). Этот квест — первый шаг к превращению "черного ящика"
в прозрачную, живую систему.
"""
