"""Квест 31.1: Создание "CLI-Артефакта".

Этот пергамент открывает "Свиток Профессиональных Инструментов". Его главная
цель (МАКРО-контекст) — превратить простой Python-скрипт в элегантный и
переиспользуемый "CLI-Артефакт" с помощью гримуара `click`.

Мы извлекаем "сущность 'Ритуала Самопроверки'" из сердца нашего API и
запечатываем ее в отдельный, самостоятельный скрипт. Затем мы обучаем его
подчиняться "Слову Мастера" — коротким, ясным командам, отданным прямо в
терминале. Декораторы `@click.command()` и `@click.option()` превращают
нашу функцию в полноценный CLI-инструмент с автоматической генерацией
помощи (`--help`), параметрами по умолчанию и возможностью передавать
конфигурацию извне.

Этот квест демонстрирует, как отделять логику от конфигурации, что является
фундаментом для автоматизации и создания профессиональных утилит.
"""
...
# --- > НОВАЯ РУНА: Это заклинание-декоратор от духа 'click',
# оно превращает нашу простую функцию в могущественную команду для терминала.
@click.command()
...
@click.option(
    ...
    "--model-name",
    # Значение по умолчанию: если Мастер не укажет иное, будет призван этот
    # дух.
    default="distilbert-base-uncased-finetuned-sst-2-english",
    # Текст помощи: мудрость, которую 'click' покажет Мастеру, если тот
    # попросит о помощи (`--help`).
    help="Имя модели из Hugging Face Hub для проведения валидации.",
)
# Наша функция теперь волшебным образом получает значение из опции
# '--model-name' в качестве аргумента.
def run_validation(model_name):
    """Загружает указанную модель, проводит валидуцию и выводит результат в консоль."""
    # Мы используем заклинание 'secho' от 'click', чтобы объявить о начале
    # ритуала сияющим желтым цветом.
    click.secho(f"[*] Призываю духа модели: '{model_name}'...", fg="yellow")

    # Мы начинаем защитный ритуал 'try', чтобы поймать любую ошибку, если дух
    # не откликнется на зов.
    try:
        # Мы произносим заклинание 'pipeline', используя имя, переданное
        # Мастером.
    ...
    # Если во время призыва произошла ошибка, наш защитный ритуал ловит ее.
        ...
        # Мы кричим об ошибке тревожным красным цветом, объясняя, что пошло не
        # так.
        click.secho(f"[!] Ошибка призыва: {e}", fg="red")
        # И немедленно прерываем ритуал, так как без духа продолжать
        # бессмысленно.
        return
    ...
    # Мы объявляем о начале испытаний, используя стандартное заклинание 'echo'
    # от 'click'.
    ...
    # Мы начинаем печатать финальный отчет, выделяя заголовок жирным шрифтом
    # для важности.
