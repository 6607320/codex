"""Квест 31.2: "Камень Истины" (валидационный набор данных).

Этот пергамент не является исполняемым ритуалом. Его единственная цель
(МАКРО--контекст) — хранить "Камень Истины" (`VALIDATION_SET`).

Этот священный эталон используется главным пергаментом `validate_model.py`
для проведения "ритуала самопроверки" и "профилирования". Он представляет
собой небольшой, но репрезентативный набор "свитков" (текстов) с заранее
известной "истинной сутью" (метками), на которых измеряется точность и
производительность "духов" (моделей).
"""
