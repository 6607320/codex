"""Квест 31.2: Продвинутая Отладка с помощью `torch.profiler`.

Этот пергамент — продолжение "Свитка Профессиональных Инструментов". Его
главная цель (МАКРО-контекст) — освоить искусство **профилирования**,
превратив догадки об "узких местах" производительности в неопровержимые
факты.

Мы берем наш CLI-артефакт из предыдущего квеста и "оборачиваем" его
"горячую" часть (цикл валидации) в могущественный "Хронометр Истинного
Времени" (`torch.profiler`). Этот инструмент позволяет нам заглянуть в
самое сердце нашего заклинания и увидеть невидимое: потоки времени и
энергии, распределенные между CPU и GPU. Этот квест доказывает, что
оптимизация начинается не с исправлений, а с точных измерений.
"""
