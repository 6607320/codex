...
# Мы призываем могущественного духа 'click', мастера командной строки и красноречия.
import click

# --- > НОВАЯ РУНА: Мы призываем сам гримуар 'torch', чтобы получить доступ к его фундаментальной магии.
import torch

# --- > НОВАЯ РУНА: Из гримуара 'torch' мы призываем его самое зоркое око — 'Хронометр' (profiler).
from torch import profiler


# --- Акт 2: Основной Ритуал Валидации ---
# Заклинание-декоратор от духа 'click', превращающее нашу функцию в команду для терминала.
@click.command()
...
# Наша функция волшебным образом получает значение из опции '--model-name' в качестве аргумента.
def run_validation(model_name):
    """
    Загружает указанную модель, профилирует валидацию и выводит результаты.
    """
    # Используем заклинание 'secho' от 'click', чтобы объявить о начале ритуала сияющим желтым цветом.
    click.secho(f"[*] Призываю духа модели: '{model_name}'...", fg="yellow")

    # Начинаем защитный ритуал 'try', чтобы поймать любую ошибку, если дух не откликнется на зов.
    try:
    ...
    # Если во время призыва произошла ошибка, наш защитный ритуал ловит ее.
        ...
        # Кричим об ошибке тревожным красным цветом, объясняя, что пошло не так.
        click.secho(f"[!] Ошибка призыва: {e}", fg="red")
        # Немедленно прерываем ритуал, так как без духа продолжать бессмысленно.
        return
    ...
    # Объявляем о начале испытаний, используя стандартное заклинание 'echo' от 'click'.
    click.echo(f"[*] Начинаю испытание на {total_samples} примерах...")

    # --- > НОВЫЕ РУНЫ: Начало ритуала "Истинного Времени" < ---
    # Мы начинаем контекст Хронометра, приказывая ему начать запись всех магических операций.
    with profiler.profile(
        # Указ 1: "Следи за активностью и разума (CPU), и Кристалла Маны (CUDA)".
        activities=[profiler.ProfilerActivity.CPU, profiler.ProfilerActivity.CUDA],
        # Указ 2: "Записывай 'форму' магических потоков (размеры тензоров), чтобы мы знали их масштаб".
        record_shapes=True,
        # Указ 3: "Записывай путь вызова каждого заклинания (стек вызовов) для глубокой отладки".
        with_stack=True,
        # Мы даем нашему запущенному Хронометру имя 'prof', чтобы обращаться к нему позже.
    ) as prof:
        # Мы помещаем 'горячую' часть нашего кода — само испытание — внутрь области действия Хронометра.
        with click.progressbar(VALIDATION_SET, label="Провожу испытание...") as bar:
            ...
            for item in bar:
                ...
                # Спрашиваем предсказание у нашего духа (эта операция теперь под пристальным надзором Хронометра).
    ...
    # Как только мы выходим из блока 'with', Хронометр автоматически останавливает запись и сохраняет все данные.
    ...
    # Начинаем печатать финальный отчет о точности, выделяя заголовок жирным шрифтом.
    ...
    # Мы обращаемся к нашему Хронометру 'prof', просим его усреднить данные по ключам (именам операций) и представить их в виде таблицы, отсортированной по общему времени CPU, и показать только 15 самых медленных.
    print(prof.key_averages().table(sort_by="cpu_time_total", row_limit=15))


# --- Акт 3: Точка Входа в Ритуал ---
# Древняя руна, которая проверяет: "Этот пергамент был запущен напрямую Мастером?".
