"""Квест 30.2: "Слепое Испытание" (A/B Тестирование).

Этот пергамент демонстрирует один из самых важных ритуалов MLOps —
A/B тестирование. Его главная цель (МАКРО-контекст) — создать "Амулет с
Развилкой", который будет вслепую сравнивать производительность двух
разных "духов" (моделей) в реальном времени.

Ключевые элементы этого ритуала:
1.  **Призыв Двух Духов:** Мы загружаем две разные модели для одной задачи.
2.  **Магическая Развилка:** В "вратах" `/analyze` мы используем "духа
    Случайности", чтобы направить запрос то к одной, то к другой модели.
3.  **Кристалл-Призма:** Мы создаем "Счетчик" (`Counter`) `Prometheus` с
    измерением (`label`) 'model_version'. При каждом запросе мы увеличиваем
    счетчик для той модели, которая его обработала.

Этот квест доказывает, как с помощью метрик с измерениями можно проводить
глубокий, сегментированный анализ, что является основой для принятия
решений о выкатке новых моделей на основе реальных данных.
"""
