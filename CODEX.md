...
- Квест 24.1: Создание "Магического Свитка" (Собрать проект в `pip`-пакет с `setup.py`) `[x]`
    <!--
    **🔮 Легенда Квеста:**
    Истинный Мастер не только создает артефакты, но и знает, как доставить их другим. В этом квесте мы осваиваем магию Пакетирования. Мы создаем "Магический Свиток" (`quest_24_1.py`) — рецепт, который позволяет "запечатать" наш проект в универсальный `.whl` файл ("колесо"). Этот файл любой другой маг сможет установить одной командой `pip install`.
    ...
    **📜 Ритуал для этого квеста:**
    ...
    2. **Написание Рецепта:** Создай файл `quest_24_1.py` (который по своей сути является рецептом `setup.py`) и опиши в нем свой проект: имя, версия, автор, и, самое главное, зависимости (`install_requires`).
...
- Квест 26.1: Настройка "Магического Стража" (Создать простой workflow в `GitHub Actions`) `[x]`
    ...
    **📜 Ритуал для этого квеста:**
    Все необходимые артефакты (`quest_26_1_ci_flake8.yaml`, `.flake8`, `spell_with_flaw.py`) **уже существуют** в Летописи. Твоя задача — **активировать** ритуал.
    ...
    2.  **Пробуждение Стража:** Запечатай это незначительное изменение в Летописи и отправь его в центральное хранилище.
        ```bash
        git add .
        git commit -m "feat(Scroll_26): Пробуждаю Стража для Квеста 26.1"
        git push
        ```
    ...
    **❓ Вопрос от Техноманта:**
    Мастер, я вижу, что Страж проверяет только папку Квеста 26.1. Как мне заставить его охранять всю цитадель целиком?

    **Ответ Мастера:**
    Твой вопрос указывает на путь к созданию **"Великого Стража"**. Сейчас мы пробудили "локального стражника". Чтобы он охранял всю Летопись, тебе нужно просто изменить руну `paths:` в `quest_26_1_ci_flake8.yaml`. Если ты уберешь ее совсем, Страж будет реагировать на **любое** изменение в **любом** `.py` файле. Выбор стратегии — охранять каждую мастерскую отдельно или всю цитадель целиком — это архитектурное решение, которое принимает Мастер-Инженер.
    -->
- Квест 26.2: Ритуал "Авто-Тестирования" (Настроить Action, который запускает `pytest` при каждом коммите) `[x]`
    ...
    **📜 Ритуал для этого квеста:**
    Все необходимые артефакты (`quest_26_2_ci_full.yaml`, `arcane_math.py`, `test_arcane_math.py`) **уже существуют** в Летописи. Твоя задача — **активировать** усовершенствованный ритуал.

    1.  **Призыв `pytest`:** Установи гримуар для тестирования в свое локальное окружение: `pip install pytest pytest-cov`.
    2.  **Локальный Экзамен:** Убедись, что твои тесты работают на твоей машине, запустив `pytest` из папки `Part_4_Engineering/Scroll_26/Quest_2/`. Ожидаемый результат: `2 passed`.
    3.  **Активация Триггера:** Открой пергамент `Part_4_Engineering/Scroll_26/Quest_2/arcane_math.py`. Добавь в конец файла пустой комментарий (`# trigger`) и сохрани его.
    4.  **Пробуждение Стража:** Запечатай это изменение в Летописи и отправь его в центральное хранилище.
        ```bash
        git add .
    ...
    **🏆 Главное озарение:**
    Ты построил полноценный **CI-пайплайн (Continuous Integration)**, который автоматически проверяет и **стиль (linting)**, и **корректность (testing)** твоего кода. Ты понял, как создавать изолированные, эволюционирующие workflow, не нарушая целостность предыдущих квестов.
