...
Этот пергамент открывает "Свиток Магических Потоков Данных" и учит
искусству "кормления" Големов. Его главная цель (МАКРО-контекст) — создать
свой собственный, кастомный конвейер данных с помощью `Dataset` и `DataLoader`.

Ритуал состоит из двух актов:
1.  **Создание "Хранителя" (`Dataset`):** Мы пишем класс, который наследуется
    от `torch.utils.data.Dataset` и реализует три священных заклинания:
    `__init__` (составляет "каталог" данных), `__len__` (сообщает их
    количество) и `__getitem__` (загружает один элемент "лениво", по запросу).
2.  **Создание "Подносчика" (`DataLoader`):** Мы "оборачиваем" нашего
    "Хранителя" в `DataLoader`, который эффективно приносит данные Голему
    "пачками" (батчами).

Этот квест демонстрирует обязательный навык для работы с любыми
нестандартными или очень большими наборами данных.
"""
...
# Из гримуара `Pillow` мы призываем заклинание `Image` для открытия
# изображений.
from PIL import Image

# Из раздела `torch.utils.data` мы призываем два главных чертежа для
# работы с данными.
...
class CustomImageDataset(Dataset):
    ...
    # Мы определяем заклинание Подсчета (`__len__`),
    #  отвечающее на вопрос "Сколько всего свитков?".
    # Оно должно быть реализовано в любом "Хранителе".
    def __len__(self):
        # `len()` — это стандартное заклинание Python, возвращающее длину списка.
        return len(self.image_paths)

    # Мы определяем заклинание "Принеси-Свиток" (`__getitem__`)
    #  — главное заклинание Хранителя.
    # Оно срабатывает, когда "Подносчик" просит свиток под конкретным номером
    # `idx`.
    def __getitem__(self, idx):
        # Хранитель заглядывает в свой "каталог" и берет путь к свитку по его
        # номеру.
        image_path = self.image_paths[idx]
        # Он "вскрывает" свиток, читая изображение с диска.
        # `.convert("RGB")` — на всякий случай превращает
        #  его в стандартный 3-канальный формат.
        ...
        # Для простоты этого квеста, мы будем использовать номер свитка как его
        # "метку".
        label = idx
...
# Мы даем ему путь к нашей "библиотеке" ('generated_palette') и наш
# конвейер "линз".
...
# 3. `shuffle=True` — мы приказываем ему перемешивать "каталог" перед
# каждым новым "учебным годом" (эпохой).
