...
Этот пергамент — практический урок по оптимизации конвейера данных. Его
главная цель (МАКРО-контекст) — наглядно, с помощью "Хронометра", доказать
и измерить эффект от использования `num_workers` в `DataLoader`.

Ритуал проводит два эксперимента. Сначала мы "пролистываем" весь датасет с
`num_workers=0` (последовательный режим), а затем — с `num_workers=4`
(параллельный режим). Чтобы разница была очевидной, мы добавляем
искусственную задержку в `__getitem__`, имитируя сложную обработку данных.
Этот квест доказывает, что параллельная загрузка — это не "опция", а
необходимость для эффективного использования GPU.
"""
