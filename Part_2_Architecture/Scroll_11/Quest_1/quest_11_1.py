...
# Мы призываем "Художника" `matplotlib.pyplot` (с псевдонимом `plt`) для рисования нашей карты.
import matplotlib.pyplot as plt

# Мы призываем `numpy` (с псевдонимом `np`) для математических операций (вычисления корня).
import numpy as np
...
# Мы призываем функциональную часть `PyTorch` (с псевдонимом `F`) для заклинания Softmax.
...
# `qkv.size(-1)` — это заклинание, получающее "глубину смысла" нашей ауры (в данном случае, 3).
d_k = qkv.size(-1)
...
# `.squeeze(0)` — убирает "батч" измерение. `.detach()` — "открепляет" от графов вычислений.
...
# Мы запускаем ритуальный цикл, чтобы в каждую ячейку нашей карты вписать ее значение в процентах.
...
print(
    "1. Найди строку 'Король'. Самая яркая (горячая) ячейка в этой строке - в столбце 'принц'."
)
