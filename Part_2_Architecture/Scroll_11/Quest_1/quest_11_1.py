...
Этот пергамент открывает "Свиток Фокуса Мысли" и демистифицирует
сердце Трансформеров — механизм Внимания (Attention). Его главная цель
(МАКРО-контекст) — не просто реализовать `scaled dot-product attention` с
нуля, а создать **визуальный эксперимент**, чтобы интуитивно понять, как
этот механизм позволяет словам "смотреть" друг на друга.

Мы создаем "ауры" (эмбеддинги) для нескольких слов и вручную, шаг за шагом,
проводим ритуал вычисления "карты внимания". Финальный артефакт — это
тепловая карта, которая наглядно показывает, как похожие по смыслу слова
"притягиваются" друг к другу. Этот квест доказывает, что `Attention` — это
элегантный математический способ измерить "смысловую близость" между
