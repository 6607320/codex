...
# Мы призываем `torch.nn.functional` (с псевдонимом `F`) — гримуар с "заклинаниями", такими как ReLU и Max Pooling.
import torch.nn.functional as F

# Мы призываем гримуар `torchvision` для работы с датасетами изображений, как MNIST.
import torchvision
...
# Второй раздел: мы призываем чертежи "рунных камней", которые мы выковали в предыдущих квестах.


# Мы призываем наш собственный чертеж Линейного слоя из Квеста 9.2.
class MyLinear(nn.Module):
    # Мы определяем ритуал сотворения, в котором мы создаем "руны" — веса и смещение.
    def __init__(self, in_features, out_features):
        # Мы произносим обязательное заклинание, призывающее дух предка (`nn.Module`).
        super().__init__()
    ...
    # Мы определяем ритуал "прямого прохода", описывающий, как руны действуют на данные.
    def forward(self, x):
        # Мы произносим главное заклинание: y = x * W^T + b, используя готовое заклинание из `F`.
        return F.linear(x, self.weight, self.bias)


# Мы призываем наш собственный чертеж Сверточного слоя из Квеста 10.1.
class MyConv(nn.Module):
    # Мы определяем ритуал сотворения, в котором мы создаем "руны" — ядро свертки и смещение.
    ...
    # Мы определяем ритуал "прямого прохода", описывающий, как ядро "сканирует" изображение.
    def forward(self, x):
        # Мы произносим главное заклинание: свертка входного изображения `x` с нашим ядром.
...
class MyManualCNN(nn.Module):
    # Мы определяем ритуал сотворения, в котором мы создаем этажи нашей "Башни".
    def __init__(self):
        # Мы произносим обязательное заклинание, призывающее дух предка.
        super().__init__()
        # Мы создаем первый этаж: нашу собственную ручную свертку, принимающую 1 канал (ч/б) и выдающую 16 карт.
        ...
        # Она принимает "сплющенные" данные и выдает 10 вердиктов (для цифр 0-9).
        self.fc1 = MyLinear(in_features=16 * 14 * 14, out_features=10)

    # Мы определяем ритуал "прямого прохода", описывающий путь сигнала через "Башню".
    def forward(self, x):
        # Шаг 1: Сигнал проходит через первый, сверточный этаж.
        x = self.conv1(x)
        # Шаг 2: Мы применяем "искру жизни" ReLU, чтобы Голем мог учиться сложным вещам.
        x = F.relu(x)
        # Шаг 3: Мы "сжимаем" карты признаков с помощью Max Pooling, оставляя только самое важное.
        x = F.max_pool2d(x, kernel_size=2, stride=2)
        # Шаг 4: Мы "сплющиваем" 2D карты признаков в один длинный вектор, чтобы "голова" могла их прочесть.
        # `-1` — это магическая руна, означающая "PyTorch, посчитай этот размер сам".
        x = x.view(-1, 16 * 14 * 14)
        # Шаг 5: Сплющенный вектор проходит через "голову", которая выносит финальный вердикт (логиты).
        x = self.fc1(x)
...
# Эта конструкция (`if __name__ == "__main__":`) — священное начало любого исполняемого пергамента.
if __name__ == "__main__":
    ...
    # Мы создаем "Духа-Подносчика", который будет подавать рукописи нашему Голему пачками по 64 штуки.
    ...
    # Мы выбираем "наставника" (алгоритм Adam), который будет корректировать "разум" Голема.
    optimizer = torch.optim.Adam(model.parameters(), lr=0.001)

    # Мы оповещаем о начале ритуала Наставления.
    print("Начинаем наставление 'Башни Прозрения', собранной вручную...")
    # Мы начинаем цикл обучения. Мы покажем Голему все рукописи 3 раза (3 эпохи).
    for epoch in range(3):
        # Мы проходим по всем пачкам рукописей. `i` — номер пачки, `data` — сама пачка.
        for i, (images, labels) in enumerate(train_loader):
            ...
            optimizer.step()
        # Мы сообщаем о завершении одной эпохи обучения и о том, насколько Голем стал умнее.
    ...
    model.eval()
    # Наш Голем смотрит на незнакомую рукопись. `unsqueeze(0)` добавляет "измерение пачки".
