...
import cv2
# Мы призываем помощника `numpy` для математических операций с нашими "комнатами" и "трафаретами".
import numpy as np
...
# Мы вычисляем размеры будущей "Карты Признаков". Она будет чуть меньше оригинала, так как "фонарь" не может выходить за края.
...
# Мы нормализуем результат, чтобы его можно было сохранить как картинку (переводим значения в диапазон 0-255).
