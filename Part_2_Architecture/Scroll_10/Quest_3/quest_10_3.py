...
transform = transforms.Compose(
    ...
    [
        # Первая "линза": превращает картинку из объекта Pillow в тензор PyTorch.
        transforms.ToTensor(),
...
class MiniCNN(nn.Module):
    ...
    def __init__(self):
        ...
        super().__init__()
        # Мы создаем первый этаж: 1 входной канал (ч/б), 16 "гномов с фонарями", трафарет 3x3.
        self.conv1 = nn.Conv2d(1, 16, kernel_size=3, stride=1, padding=1)
        # Мы создаем второй этаж: 16 входных карт, 32 "гнома", трафарет 3x3.
        self.conv2 = nn.Conv2d(16, 32, kernel_size=3, stride=1, padding=1)
        # Мы создаем "Магистра-Оценщика": он принимает "сплющенный" атлас и выдает вердикт по 10 классам (цифры 0-9).
        self.fc1 = nn.Linear(32 * 7 * 7, 10)

    # Мы определяем ритуал "прямого прохода", описывающий путь сигнала через "Башню".
    def forward(self, x):
        # Мы прогоняем образ через 1-й этаж, "магический переключатель" ReLU и "уменьшитель" MaxPool.
...
# Мы сотворяем Голема по нашему чертежу и отправляем его на Кристалл Маны (GPU).
...
model.train()
# Мы запускаем цикл по всем пачкам рукописей, оборачивая его в "индикатор прогресса".
...
# Мы находим вердикт с самой высокой уверенностью и извлекаем его числовое значение.
