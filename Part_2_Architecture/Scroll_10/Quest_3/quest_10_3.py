...
Этот пергамент знаменует переход от создания "атомов магии" к сборке
целого "магического механизма". Его главная цель (МАКРО-контекст) —
спроектировать, собрать и обучить свою первую Сверточную Нейронную Сеть
(CNN) — "Башню Прозрения".

В отличие от предыдущего квеста, здесь мы используем готовые,
профессиональные "строительные блоки" из гримуара PyTorch (`nn.Conv2d`,
`nn.Linear`), чтобы создать Голема, способного "читать" рукописные
цифры MNIST. Этот ритуал закрепляет полный цикл работы Архитектора:
Проектирование -> Сборка -> Обучение -> Оценка.
"""
