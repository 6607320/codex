...
Этот пергамент открывает "Свиток Творения из Ничего". Его главная цель
(МАКРО-контекст) — реализовать с нуля наш первый артефакт, способный
**творить**: "Генератор Снов" (Variational Autoencoder - VAE).

Модель состоит из двух частей: "Магического Пресса" (Энкодер), который
учится "сжимать" суть рукописных цифр в крошечное "пространство снов", и
"Проектора Снов" (Декодер), который учится "видеть сны" — генерировать
новые, уникальные цифры из случайных точек этого пространства. Этот квест
демистифицирует архитектуру автоэнкодеров и их "вариационную" магию,
включая составную функцию потерь (BCE + KLD).
"""
...
latent_dim = 2
# Мы устанавливаем количество "учебных лет" (полных проходов по всему
# учебнику).
epochs = 10
# Мы устанавливаем количество "страниц" в одной пачке, которую мы
# показываем ученику за раз.
batch_size = 128
...
# картинку из формата Pillow в Тензор PyTorch и нормализует пиксели к
# диапазону [0, 1].
...
# Мы создаем чертеж нашего артефакта, который состоит из двух частей:
# Пресса и Проектора.
class VAE(nn.Module):
    ...
    def __init__(self):
        ...
        # Первый слой: принимает "расплющенную" картинку (28*28=784 пикселя) и
        # сжимает до 400.
        self.fc1 = nn.Linear(784, 400)
        # Второй слой (параллельный): из 400 "мыслей" создает "средние" координаты
        # жемчужины (mu).
        self.fc21 = nn.Linear(400, latent_dim)
        # Третий слой (параллельный): из тех же 400 "мыслей" создает "разброс"
        # жемчужины (logvar).
        self.fc22 = nn.Linear(400, latent_dim)

        # --- Чертеж "Проектора Снов" (Декодер) ---
        # Первый слой: принимает "жемчужину" (2 числа) и "распаковывает" ее до 400
        # "мыслей".
        self.fc3 = nn.Linear(latent_dim, 400)
        # Второй слой: из 400 "мыслей" воссоздает полную "расплющенную" картинку
        # на 784 пикселя.
        self.fc4 = nn.Linear(400, 784)

    # Мы определяем заклинание "Сжатия", описывающее работу "Пресса".
    def encode(self, x):
        # Мы пропускаем данные через первый слой и "магический переключатель"
        # ReLU.
        h1 = F.relu(self.fc1(x))
        # Мы возвращаем ДВА результата: "средние" координаты (mu) и "разброс"
        # (logvar).
        return self.fc21(h1), self.fc22(h1)

    # Мы определяем заклинание "Магической Случайности" (Репараметризационный
    # трюк).
    def reparameterize(self, mu, logvar):
        ...
        # Мы создаем финальную "жемчужину",
        #  смещая "среднюю" точку на случайную величину.
        # Это позволяет "пространству снов" быть гладким и непрерывным.
        return mu + eps * std

    # Мы определяем заклинание "Восстановления", описывающее работу
    # "Проектора".
    def decode(self, z):
    ...
    # Мы определяем главное заклинание, описывающее полный цикл
    # "сжатие-восстановление".
    def forward(self, x):
        # Мы сжимаем исходный образ (`x.view(-1, 784)` "расплющивает" его) и
        # получаем mu и logvar.
        mu, logvar = self.encode(x.view(-1, 784))
        # Мы создаем "жемчужину" с элементом случайности.
        z = self.reparameterize(mu, logvar)
        # Мы восстанавливаем образ из "жемчужины" и возвращаем его вместе с mu
        # и logvar.
...
print(
    "Сон материализован! Открой 'dreams/dream_sample.png', "
    "чтобы увидеть новые, сотворенные цифры."
)
