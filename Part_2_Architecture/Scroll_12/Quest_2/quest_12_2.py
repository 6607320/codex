...
Этот пергамент — самый амбициозный ритуал "Школы Архитектуры". Его главная
цель (МАКРО-контекст) — реализовать с нуля упрощенную версию Диффузионной
Модели (DDPM), которая лежит в основе `Stable Diffusion` и `Midjourney`.

Ритуал состоит из двух великих частей:
1.  **Наставление:** Мы учим "Скульптора Хаоса" (`SimpleUNet`) видеть
    порядок в шуме, предсказывая "пыль" (шум), которая была добавлена к
    идеальному образу.
2.  **Творение:** Мы даем обученному "Скульптору" бесформенный кусок
    хаоса и просим его шаг за шагом "счищать пыль", пока не проявится
    новый, уникальный образ.

Этот квест демистифицирует суть современной генеративной магии, доказывая,
что творение — это на самом деле ритуал "очищения от шума".
