...
Этот пергамент — один из двух артефактов Квеста 9.1. Его главная цель
(МАКРО-контекст) — продемонстрировать и демистифицировать процесс обучения
нейронной сети (обратное распространение ошибки) для простейшей модели.

Мы вручную, шаг за шагом, реализуем "пристрелку" или градиентный спуск:
1. Делаем предсказание (прямой проход).
2. Измеряем ошибку (loss).
3. Вручную вычисляем "шепот исправления" (градиенты).
4. Вручную обновляем "руны" (веса).

Этот ритуал служит эталоном для сравнения с автоматизированной магией
PyTorch Autograd, представленной в `quest_9_1_manual.py`.
"""
...
# Мы создаем тензор для "множителя" `w`, отключая автоматическое
# отслеживание градиентов.
w = torch.tensor(
    # Мы устанавливаем начальное значение 'множителя' на ноль.
    [0.0],
    # Мы явно говорим PyTorch: "Не следи за этой руной, мы будем крутить ее
    # сами".
    requires_grad=False,
)
...
# решительно мы будем крутить 'ручки'.
learning_rate = 0.01
...
for epoch in range(15):
    ...
    # Мы вычисляем предсказание, используя текущие значения 'множителя' и
    # 'смещения'.
    y_predicted = w * X + b
    ...
    # Мы вычисляем 'шепот' для 'множителя' `w` и извлекаем его числовое
    # значение.
    grad_w = (2 * (y_predicted - Y) * X).item()
    # Толкование следующей руны.
    # "Шепот" для ручки 'b': насколько сильно 'b' повлияла на ошибку.
    # Мы вычисляем 'шепот' для 'смещения' `b` и извлекаем его числовое
    # значение.
    ...
    # Мы обновляем 'множитель' `w`, двигаясь в сторону, противоположную
    # 'шепоту'.
    ...
    print(
        f"Эпоха {epoch + 1}: Предсказание={y_predicted.item():.4f}, "
