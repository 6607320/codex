"""Квест 9.1: Ручное заклинание градиента ("Истинный Ручной Режим").

Этот пергамент — один из двух артефактов Квеста 9.1. Его главная цель
(МАКРО-контекст) — полностью демистифицировать магию `loss.backward()`,
реализовав весь процесс обратного распространения ошибки (backpropagation)
вручную, на чистой математике.

Мы не используем `autograd` или `optimizer`. Вместо этого, мы "пристреливаем
магический лук", шаг за шагом вычисляя производные (градиенты) и вручную
обновляя "руны" (`w` и `b`). Этот ритуал доказывает, что могущественные
заклинания PyTorch — это лишь автоматизация простых и понятных
...
for epoch in range(15):
    ...
    # Делаем выстрел по формуле: попадание = (мишень * сила_натяжения)
    #  + поправка_на_ветер
    ...
    print(
        ...
        f"Выстрел {epoch + 1}: Попадание={y_predicted:.4f}, "
        f"Промах={loss:.4f}, w={w:.4f}, b={b:.4f}"
    )
