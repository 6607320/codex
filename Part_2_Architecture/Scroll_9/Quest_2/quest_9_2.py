...
Этот пергамент знаменует переход от "заклинаний" к ковке "артефактов".
Его главная цель (МАКРО-контекст) — понять, как устроены "строительные
блоки" (`nn.Module`) в PyTorch, создав свой собственный аналог `nn.Linear`
с нуля.

Мы создаем "Рунный Камень" (`MyLinear`) — самодостаточный класс, который
инкапсулирует (прячет внутри себя) и свои "руны" (`weight`, `bias`), и
логику их использования (`forward`). Этот квест закладывает фундаментальное
понимание объектно-ориентированной магии PyTorch, необходимое для
построения любых сложных архитектур.
"""
...
# Мы призываем `nn` — главу гримуара PyTorch с базовыми чертежами для всех
# нейронных сетей.
...
class MyLinear(nn.Module):
    ...
    # Оно срабатывает в момент "сотворения" камня. Его задача — создать
    # "гнезда" для рун.
    def __init__(self, input_size, output_size):
        # `super().__init__()` — это обязательная руна,
        #  пробуждающая магию родительского класса.
        super().__init__()

        # Мы создаем наши "руны" `w` и `b`.
        # `nn.Parameter` — это специальный "магический пергамент",
        #  который говорит PyTorch:
    ...
    def forward(self, x):
        ...
        # `.T` — это заклинание транспонирования,
        #  техническая деталь для правильного умножения.
...
# Он будет принимать на вход 1 число (`input_size=1`) и выдавать на выход
# 1 число (`output_size=1`).
