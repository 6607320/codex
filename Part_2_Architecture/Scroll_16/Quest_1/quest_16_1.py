...
Этот пергамент открывает "Свиток Хроник Экспериментов" и вводит нас в мир
MLOps. Его главная цель (МАКРО-контекст) — установить и настроить `MLflow` —
наш "Лабораторный Журнал" для отслеживания экспериментов.

Ритуал не обучает модель. Вместо этого он демонстрирует три фундаментальных
заклинания `MLflow`:
1.  `mlflow.log_param()`: для записи "ингредиентов" ритуала (гиперпараметров).
2.  `mlflow.log_metric()`: для записи "результатов" (метрик).
3.  `mlflow.log_artifact()`: для сохранения "артефактов" (любых файлов).

Этот квест доказывает, что `MLflow` превращает хаос экспериментов в
организованную, воспроизводимую науку.
"""
...
# `with` — конструкция, которая гарантирует,
#  что когда ритуал закончится (код выйдет из этого блока),
...
with mlflow.start_run(run_name="Пробный Ритуал"):
    ...
    # Параметр — это "настройка" твоего ритуала, которую ТЫ задаешь
    ...
    # Артефакт — это любой файл, который ты хочешь прикрепить
    #  к своей "странице" журнала.
...
print(
    "\nЗатем открой в браузере адрес, который он тебе покажет "
    "(обычно http://12.0.0.1:5000)."
)
