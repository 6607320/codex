...
# Мы устанавливаем 'Скорость Обучения' — это определяет, насколько решительно мы будем крутить 'руны' после каждого промаха.
learning_rate = 0.01
...
# Мы запускаем ритуальный цикл, который повторится 15 раз — это будет 15 наших 'выстрелов'.
for epoch in range(15):
    ...
    # Мы вычисляем 'промах' (ошибку) — квадрат разницы между попаданием и центром мишени.
    ...
    # Мы вычисляем 'шепот' для 'Силы Натяжения' — это производная функции потерь по `w`.
    grad_w = 2 * (y_predicted - y) * x
    # Мы вычисляем 'шепот' для 'Поправки на Ветер' — производная функции потерь по `b`.
    ...
    # Мы обновляем 'Силу Натяжения', двигаясь в сторону, противоположную 'шепоту', с силой, равной 'Скорости Обучения'.
