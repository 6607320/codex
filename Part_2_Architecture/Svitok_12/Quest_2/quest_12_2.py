...
# Мы задаем размер наших будущих творений. 32x32 — хороший компромисс для скорости.
image_size = 32
# Мы устанавливаем количество "учебных страниц" в одной пачке.
batch_size = 128
# Мы устанавливаем количество "учебных лет". Диффузия требует больше времени на обучение.
epochs = 20
...
transform = transforms.Compose(
    ...
    [
        # Мы увеличиваем размер цифр MNIST (28x28) до нашего рабочего размера (32x32).
...
# Мы устанавливаем, что у нас будет 300 шагов от чистой статуи до полного хаоса.
timesteps = 300
...
# Мы определяем заклинание, которое может "испортить" любую картинку до любого шага 't' за один раз.
def get_noisy_image(x_start, t):
    ...
    # Мы применяем формулу "зашумления": (корень_альфы * чистый_образ) + (корень_не_альфы * шум).
...
# Мы создаем "Рулетку", измеряющую разницу между двумя образами (предсказанным и реальным шумом).
...
for epoch in range(epochs):
    ...
    for step, (images, _) in tqdm(
        ...
        # Мы выбираем случайный "шаг зашумления" 't' для каждой картинки в пачке.
        t = torch.randint(0, timesteps, (images.size(0),)).long()
        # Мы отправляем чистые образы на Кристалл Маны.
        images = images.to("cuda")

        # Мы "портим" чистые образы до состояния 't' и запоминаем, какой шум мы добавили.
        ...
        # Мы измеряем ошибку: насколько предсказанный шум отличается от реального.
...
# Мы запускаем цикл "очищения" в ОБРАТНОМ порядке — от самого шумного шага к чистому.
for t in tqdm(range(timesteps - 1, -1, -1), desc="Творение из хаоса"):
    ...
    with torch.no_grad():
        ...
        if t > 0:
            ...
            # Мы добавляем его к "очищенному" образу с правильным коэффициентом.
