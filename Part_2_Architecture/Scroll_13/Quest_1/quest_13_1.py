...
Этот пергамент открывает "Свиток Архитектуры Экономии". Его главная цель
(МАКРО-контекст) — не просто использовать, а **создать с нуля** "экономный"
строительный блок `Depthwise Separable Convolution`.

Это "секретный ингредиент", который делает модели для мобильных устройств
(как `MobileNet`) быстрыми и легкими. Ритуал разделяет работу на два этапа:
"гномы-специалисты" (`depthwise`) ищут узоры, а "магистр-алхимик"
(`pointwise`) их смешивает. Финальный скрипт численно доказывает,
что наш артефакт в несколько раз эффективнее по количеству параметров,
чем стандартный `nn.Conv2d`, при сохранении той же формы выхода.
"""
