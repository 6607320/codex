...
Этот пергамент открывает "Свиток Искусства Сжатия". Его главная цель
(МАКРО-контекст) — освоить Post-Training Static Quantization. Это
алхимический ритуал, который "сжимает" обученную модель, превращая ее
"тяжелые золотые гирьки" (`float32`) в "легкие деревянные фишки" (`int8`).

Мы проводим полный цикл этого ритуала:
1. Обучаем "золотого" Голема (FP32).
2. Подготавливаем его к "трансмутации" (`torch.quantization.prepare`).
3. "Калибруем" на реальных данных, чтобы "алхимик" понял диапазоны "мыслей".
4. Конвертируем (`torch.quantization.convert`) в "деревянного" Голема (INT8).

Финальный акт — численное сравнение веса двух артефактов, доказывающее,
что модель стала в ~4 раза меньше, почти не теряя в мудрости.
"""

# Мы призываем "Духа-Архивариуса" `os` для работы с файлами (измерения
# размера).
import os
...
class MiniCNN(nn.Module):
    # `def __init__`: Мы определяем Заклинание Инициализации,
    #  которое создает "внутренности" Голема.
    def __init__(self):
    ...
    def forward(self, x):
        # `x = F.relu(...)`: Мы прогоняем мысль через "этаж",
        #  "переключатель" и "уменьшитель".
...
#  Мы создаем новый, "чистый" экземпляр Голема для трансмутации.
model_to_quantize = MiniCNN()
# `model_to_quantize.load_state_dict(...)`:
#  Мы копируем "разум" из обученного Голема в новый.
model_to_quantize.load_state_dict(model_fp32.state_dict())
# `model_to_quantize.eval()`: Мы переводим Голема в режим "экзамена".
model_to_quantize.eval()
# `model_to_quantize.qconfig = ...`:
#  Мы "ставим на верстак 'Алхимический Стол'" (применяем конфиг).
model_to_quantize.qconfig = torch.quantization.get_default_qconfig("fbgemm")
# `torch.quantization.prepare(...)`:
#  Мы "расставляем магических наблюдателей" внутри Голема.
...
for i, (data, _) in enumerate(train_loader):
    ...
    if i >= 10:
        # Мы прерываем цикл.
        break
    # `model_to_quantize(data)`: Мы "прогоняем" данные через Голема,
    #  чтобы "наблюдатели" собрали статистику.
    model_to_quantize(data)

# --- Трансмутация ---
# `torch.quantization.convert(...)`:
#  Мы произносим Главное заклинание! Превращаем "золото" в "дерево".
