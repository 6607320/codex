...
Этот пергамент завершает "Свиток Искусства Сжатия", обучая магии
"универсальности". Его главная цель (МАКРО-контекст) — "перевести" Голема,
созданного в "Кузнице PyTorch", на "магическую латынь" — универсальный
формат ONNX.

Ритуал состоит из двух актов:
1.  **Экспорт:** Мы используем заклинание `torch.onnx.export`, чтобы
    "подсмотреть" за работой нашей модели на "болванке" (`dummy_input`) и
    "заморозить" ее вычисления в виде статичного, переносимого "Свитка"
    `.onnx`.
2.  **Проверка:** Мы призываем "Универсального Чтеца" (`onnxruntime`), чтобы
    прочитать этот "Свиток" и выполнить предсказание, доказывая, что наш
    артефакт теперь может "жить" вне мира PyTorch.

Этот квест демонстрирует ключевой навык для развертывания (deployment)
моделей в реальных продуктах.
"""
