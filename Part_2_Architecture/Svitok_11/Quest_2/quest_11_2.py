...
# Имя этого пергамента, хранящего ритуал визуализации "Совета Мудрецов".
...
# Мы призываем "Художника" `matplotlib.pyplot` (с псевдонимом `plt`) для рисования карт.
import matplotlib.pyplot as plt

# Мы призываем `numpy` (с псевдонимом `np`) для удобной работы с массивами для рисования.
import numpy as np
...
class MultiHeadAttention(nn.Module):
    # Мы определяем заклинание Инициализации, которое создает все внутренние механизмы.
    def __init__(self, d_model, num_heads):
        # Мы произносим обязательное заклинание, пробуждающее магию родительского класса.
        super().__init__()
        # Мы проверяем, что общую "глубину смысла" можно поровну разделить между "мудрецами".
        ...
        # Мы создаем финальный "Рунный Камень" для объединения выводов всех "мудрецов".
    ...
    def scaled_dot_product_attention(self, Q, K, V):
        ...
        # Мы смешиваем "коктейль" из Значений (V), взвешенных на проценты внимания.
    ...
    def split_heads(self, x):
        ...
        # Мы "нарезаем" и "переставляем" оси, чтобы у каждого мудреца была своя стопка смыслов.
    ...
    def forward(self, Q, K, V):
        # Шаг 1: Мы создаем Q, K, V, пропуская исходные ауры через "Рунные Камни".
...
# Мы запускаем ритуал, передавая одни и те же ауры на все три входа (Self-Attention).
...
# Мы создаем большую "картину", разделенную на 8 "холстов" (2 строки, 4 столбца).
