...
class RecommendationNet(nn.Module):
    ...
    def __init__(self, num_users, num_items, embedding_dim=5):
        ...
        # Он как магическая книга: даешь ему номер искателя, а он выдает его "ауру" (вектор).
        self.user_embeddings = nn.Embedding(num_users, embedding_dim)
        
        # Создаем "Каталог Аур Артефактов". Работает так же, как и для искателей.
    ...
    def forward(self, user_ids, item_ids):
        ...
        # Главное магическое действие: мы вычисляем "резонанс" (скалярное произведение) между аурами.
...
if __name__ == "__main__":
    ...
    # Создаем "словарь" для перевода имен искателей в уникальные номера (0, 1, 2...).
    ...
    # Добавляем в нашу таблицу новую колонку с числовыми индексами для искателей.
    df['user_id'] = df['user'].map(user_to_idx)
    # Добавляем в нашу таблицу новую колонку с числовыми индексами для артефактов.
    ...
    # Призываем нашего Голема-Картографа по чертежу и отправляем на магический алтарь.
    model = RecommendationNet(num_users, num_items, embedding_dim).to(DEVICE)
    # Выбираем "меру ошибки" (Среднеквадратичная ошибка), чтобы оценивать промахи в рейтингах.
    ...
    for i in range(epochs):
        ...
        optimizer.zero_grad()
        # 'predictions = model(...)' - Голем смотрит на пары (искатель, артефакт) и предсказывает их "резонанс".
        predictions = model(user_ids, item_ids)
        # 'loss = ...' - Сравниваем предсказания с истинными рейтингами.
        loss = loss_function(predictions, ratings)
        # 'loss.backward()' - Вычисляем, как нужно изменить "ауры", чтобы уменьшить ошибку.
        loss.backward()
    ...
    # Ритуал завершен. Теперь мы можем заглянуть внутрь голема и увидеть "ауры", которым он научился.
    print("\n--- Откровение: Изученные 'Магические Ауры' (Эмбеддинги) ---")
    
    # Извлекаем финальные, обученные ауры искателей.
    # '.weight' - это сама "книга" с аурами, '.data' - извлекает из нее тензор без информации о градиентах.
    ...
    for name, idx in user_to_idx.items():
        ...
        # Выводим имя и его ауру. 'np.round(..., 2)' округляет числа для красоты.
