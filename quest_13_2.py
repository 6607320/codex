...
class TeacherModel(nn.Module):
    ...
    def forward(self, x): # Путь мысли через разум Магистра.
        ...
        x = F.relu(F.max_pool2d(self.conv2(x), 2)) # Проход через 2-й этаж, "переключатель" и "уменьшитель".
...
optimizer_teacher = optim.Adam(teacher.parameters(), lr=0.01) # Готовим "Ключ" для его обучения.
...
optimizer_student = optim.Adam(student.parameters(), lr=0.01) # Готовим "Ключ" для него.
...
for data, target in tqdm(train_loader, desc="Обучение Подмастерья (1 эпоха)"):
    ...
    distillation_loss = nn.KLDivLoss(reduction='batchmean')(
        F.log_softmax(student_logits / distillation_temp, dim=1), # Смягчаем и логируем "мысли" ученика.
        F.softmax(teacher_logits / distillation_temp, dim=1)  # Смягчаем "шепот" учителя.
    )
    ...
    loss = alpha * distillation_loss * (distillation_temp**2) + (1. - alpha) * student_loss
