...
prompt_ids = tokenizer(train_prompt, return_tensors="pt", padding="max_length", truncation=True, max_length=tokenizer.model_max_length).input_ids.to("cuda")

# Начинаем урок, который повторится 100 раз (эпох).
for epoch in range(num_train_epochs):
    for step, batch in tqdm(enumerate(train_dataloader), total=len(train_dataloader), desc=f"Эпоха {epoch+1}/{num_train_epochs}"):
        with torch.no_grad():
