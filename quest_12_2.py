...
class SimpleUNet(nn.Module):
    ...
    def forward(self, x):
        ...
        )  # Этаж 2 (сначала уменьшаем, потом анализируем)
        # Путь наверх...
        x3 = F.relu(
            self.up1(self.upsample(x2))
        )  # Этаж 3 (сначала увеличиваем, потом анализируем)
...
)  # Создаем плавный переход силы шума.
alphas = 1.0 - betas
alphas_cumprod = torch.cumprod(
    alphas, axis=0
)  # Накопленное произведение альф, ключевой параметр.
