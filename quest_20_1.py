...
# 'matplotlib.pyplot' (plt) - наш верный "магический художник" для визуализации данных и создания графиков.
import matplotlib.pyplot as plt

# --- Часть I: Импорт Магических Гримуаров ---
# 'numpy' (np) - наш магический калькулятор для создания числовых потоков и работы с массивами.
import numpy as np

# 'pandas' (pd) - гримуар для работы с "летописями времени". Он позволяет удобно хранить данные с датами в табличной форме.
import pandas as pd
...
if __name__ == "__main__":
    ...
    # Задаем, на сколько шагов во времени мы хотим заглянуть. 365 дней - один магический год.
    num_steps = 365
    # Создаем ось времени - простой массив чисел от 0 до 364 с помощью заклинания 'arange' из гримуара numpy.
    ...
    # Мы умножаем каждый день (0, 1, 2...) на небольшой коэффициент 0.1, чтобы рост был плавным.
    trend = 0.1 * time
    ...
    # 'time / 365 * 2 * np.pi' - формула, которая заставляет синус совершить один полный оборот за 365 шагов.
    # Умножаем результат на 10, чтобы сделать "колебания" более заметными (амплитуда).
    ...
    # 'np.random.normal' - заклинание, создающее шум, распределенный по "закону Гаусса" (большинство значений около нуля).
    # '0' - среднее значение шума, '2' - его "разброс" (стандартное отклонение), 'size' - количество точек шума.
    ...
    # Мы складываем все три компонента (Тренд, Сезонность и Шум) для каждой точки времени,
    ...
    # 'start' - начальная дата, 'periods' - количество дней, которое нужно сгенерировать.
    dates = pd.date_range(start="2024-01-01", periods=num_steps)

    # Создаем DataFrame - магическую таблицу.
    # Это словарь, где ключи ('date', 'value') станут названиями колонок, а значения - их содержимым.
    ...
    # 'index=False' - важная руна, которая говорит "не сохраняй внутренние номера строк pandas в файл".
    ...
    # 'plt.plot' - главная команда "нарисуй линию", соединяющую точки (даты по оси X, значения по оси Y).
